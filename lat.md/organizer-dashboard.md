...
Both the organizer and cohost layout routes render [[apps/wodsmith-start/src/components/competition-dashboard-shell.tsx#CompetitionDashboardShell]], which owns the shared chrome: sidebar, breadcrumb, and CompetitionHeader. Passing `cohostPermissions` switches it to the cohost variant — permission-filtered sidebar, a non-linked "Co-Host" breadcrumb root, and `showOrganizerActions={false}` on the header (hides the Edit and Go to Series buttons, which target organizer-only routes). [[apps/wodsmith-start/src/components/competition-sidebar.tsx#CompetitionSidebar]] is the single sidebar for both dashboards (the separate `cohost-sidebar.tsx` was deleted): each nav item declares an optional `cohostPermission` key from `CohostMembershipMetadata`; cohost mode shows an item only when the cohost holds that permission, and items without a key (Event divisions, Invites, Broadcasts, Co-Hosts, Settings, Danger zone) are organizer-only. New organizer nav items therefore appear for cohosts as soon as a permission key is assigned.
...
The page body lives in the shared [[apps/wodsmith-start/src/routes/compete/organizer/$competitionId/-pages/results-page.tsx#ResultsPage]], rendered by both the organizer and cohost results routes. The cohost route keeps its own graceful-degradation loader and injects a cohost `saveScore` callback, an `onPublishDivisionResults` callback, and a `submissionsRoute` link target pointing at the cohost submissions review route.
...
Cohost routes render the same page-body components as organizer routes so the two trees stay in sync automatically.

Page bodies live in `routes/compete/organizer/$competitionId/-pages/` (e.g. `scoring-page.tsx`, `divisions-page.tsx`, `athletes-page.tsx`, `results-page.tsx`, `leaderboard-preview-page.tsx`, `revenue-page.tsx`, `pricing-page.tsx`, `schedule-page.tsx`, `coupons-page.tsx`, `volunteers-page.tsx`, `events/events-page.tsx`, `events/event-detail-page.tsx`, `events/submission-review-page.tsx`). The event `$eventId.tsx` layout routes are loader-plus-`Outlet` only, so they are not extracted; the cohost layout returns the same loader-data shape as the organizer layout except `eventDivisionMappings` (organizer-only — no cohost server fn exists), which `EventDetailPage` treats as optional. Each route file keeps its own loader (cohost loaders keep cohost server fns, `competitionTeamId`, and `FORBIDDEN` catch defaults) and renders the shared page in a thin `RouteComponent`. Cohost-specific behavior is injected via props: mutation callbacks/overrides forwarded to the inner shared components, `routePrefix` for link targets, targeted flags like `isCohost` or `hidePayoutSetupLink` for permission-driven copy/UI, and optional props for organizer-only sections (e.g. `seriesMappingStatus`, or `volunteerWaiverStatus` on `VolunteersPage` — the cohost loader cannot fetch waiver statuses, so the waiver columns hide when omitted). Pages whose mutations live in the page body itself rather than an inner component (e.g. `CouponsPage`) accept an `overrides` bundle that replaces the default organizer server fns for reads and mutations. Prop types are derived from the inner components via `ComponentProps` rather than redeclared. Where the two loaders return different shapes, the cohost loader maps to the organizer's shape rather than the page accepting both: the cohost results loader aggregates its per-event `cohostGetEventSubmissionsFn` rows (same fn, same `FORBIDDEN` catch) into the `{total, reviewed, pending}` counts record that the organizer's `getSubmissionCountsByEventFn` returns, counting a score's verification status past `pending` as reviewed.

The submission review detail page ([[apps/wodsmith-start/src/routes/compete/organizer/$competitionId/-pages/events/submission-review-page.tsx#SubmissionReviewPage]], with verification controls and review-note helpers split into `-pages/events/submission-review/`) is rendered by both `events/$eventId/submissions/$submissionId` routes. It takes a `SubmissionReviewOverrides` bundle (verify/adjust/invalid score, audit-log delete, mark/unmark reviewed, review-note CRUD — each defaulting to the organizer server fn) plus a `submissionsListRoute` back-link target. Organizer-only sections are optional props: `siblings` (multi-video team tabs — no cohost sibling fn; when omitted the page renders the single submission's video), `videoLinksEditor` (calls `updateSubmissionVideoUrlFn` directly), and `enableManualScoreEntry` (the `EnterScoreForm` calls `enterSubmissionScoreFn` directly; cohosts pass false). The cohost route tags its per-submission notes with `videoSubmissionId` to match the organizer's registration-wide notes shape, and its verify override drops `adjustedRoundScores` since `cohostVerifySubmissionScoreFn` has no multi-round support — moot because cohost submission data never includes round scores.
