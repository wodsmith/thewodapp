...
Resend caps email sends at 5 per second account-wide, so the broadcast queue consumer is deliberately constrained to keep aggregate send rate well under that ceiling.

The consumer is pinned to `maxConcurrency: 1` and `batchSize: 1` in [[apps/wodsmith-start/alchemy.run.ts]]. This is load-bearing: Cloudflare Queues autoscales consumers up to 250 concurrent invocations by default, so any per-invocation throttle is meaningless without capping concurrency. With `maxConcurrency: 1`, only one Worker invocation processes broadcast messages at any moment, and the in-process `SEND_DELAY_MS = 250` in [[apps/wodsmith-start/src/server/broadcast-queue-consumer.ts]] translates to a global ~4 emails/second — leaving headroom for clock skew and Resend's token-bucket edges. One queue message carries ~100 recipients, so a full message finishes in roughly 25s, comfortably inside the Worker CPU wall-clock ceiling. If Resend still returns HTTP 429 (e.g. another app sharing the account), the consumer persists any successes, calls `message.retry({ delaySeconds: 10 })`, and lets Cloudflare re-deliver after the rate-limit window drains; Resend's per-recipient `Idempotency-Key` combined with the `alreadySent` DB filter prevents double-sends on retry. Poison messages exhaust `maxRetries: 3` and land in `broadcast-email-dlq-${stage}` for inspection instead of silently disappearing.
