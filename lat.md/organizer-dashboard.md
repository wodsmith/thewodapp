...
Each scored cell on the preview is also a click-through to the submission review page (`/compete/organizer/{competitionId}/events/{eventId}/submissions/{submissionId}`), and carries a compact badge showing review state so organizers can see what's left to review without opening every cell.
