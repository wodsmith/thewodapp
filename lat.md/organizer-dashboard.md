...
[[apps/wodsmith-start/src/server-fns/broadcast-fns.ts#applyAthleteWaiverFilters]] validates every selected waiver still belongs to the competition and is required for athletes, then batch-loads [[packages/wodsmith-db/src/schemas/waivers.ts#waiverSignaturesTable]] by recipient user id. Unlike question-answer filters, waiver status is per athlete and is never inherited from a captain. Pending invite recipients also match accepted guest pre-signatures on that exact invitation and required waiver. Missing or malformed proof remains unsigned; account signatures remain user-scoped. See [[series-attempt-integrity#Accepted guest pre-signatures]].
