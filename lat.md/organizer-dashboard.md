...
Optional AI-augmented flow that proposes judge rotations for one event at a time, gated behind the `AI_HEAT_SCHEDULING_ENABLED` env flag and `OPENAI_API_KEY` ([[apps/wodsmith-start/src/lib/env.ts#isAiHeatSchedulingEnabled]]).

The agent is built on Mastra (`@mastra/core/agent`) with three intent-based tools: `getEventSchedulingContext` and `getJudgeRoster` for read access, and `proposeRotation` as the only "mutation" — proposals are buffered server-side in a `ProposalCollector` and never written to the database from the agent. The organizer reviews proposals in the UI and accepts them via the existing `createJudgeRotationFn` flow, so AI-generated rotations land through the same validation and publish gate as hand-built ones. Pure scheduling logic ([[apps/wodsmith-start/src/server/ai/judge-scheduler/context.ts#buildSchedulingContext]], [[apps/wodsmith-start/src/server/ai/judge-scheduler/validation.ts#validateProposal]], [[apps/wodsmith-start/src/server/ai/judge-scheduler/projector.ts#projectCoverage]], [[apps/wodsmith-start/src/server/ai/judge-scheduler/collector.ts#ProposalCollector]]) is TDD-driven and reuses the existing `expandRotationToAssignments` / `calculateCoverage` utilities (now typed against a `RotationLike` structural shape so AI code can pass plain objects without DB-only fields). Availability mismatches are a soft constraint: the agent must mark `confidence='low'` and explain in `reason`, but the validator does not block. Per-request scoping (the proposal collector and the per-event `SchedulingContext`) is passed via Mastra's `RequestContext`, so module-cached agent state cannot leak across requests or events.

#### Streaming + UI

Live proposals stream from the API route at [[apps/wodsmith-start/src/routes/api/judge-scheduler/suggest.ts]] using Server-Sent Events. The route reuses [[apps/wodsmith-start/src/server/ai/judge-scheduler/load-context.ts#loadEventContext]] to gather heats / judges / rotations, then injects a [[apps/wodsmith-start/src/server/ai/judge-scheduler/streaming-collector.ts#BroadcastingProposalCollector]] (a subclass of `ProposalCollector` that emits `proposal` / `conflict` events for each tool call) into the agent. A leading `init` event hands the UI the judge roster and starting coverage so the panel can resolve `membershipId → displayName` without a second roundtrip; a final `done` event sends the post-acceptance coverage projection. Non-streaming callers (tests, scripted use) can still hit the equivalent `generateJudgeRotationSuggestionsFn` server function for a single batched response. The proposal-review panel ([[apps/wodsmith-start/src/components/ai/judge-scheduler/ai-suggestion-panel.tsx#AiSuggestionPanel]] + [[apps/wodsmith-start/src/components/ai/judge-scheduler/proposal-card.tsx#ProposalCard]]) parses the SSE stream via `useStreamingProposals` (POST + `ReadableStream`, supports `AbortController`-based cancel), renders one card per proposal as it arrives with judge name / heat range / lane pattern / reason / confidence chip, and offers per-proposal Accept / Discard plus a sequential "Accept all" that calls `createJudgeRotationFn` one rotation at a time so buffer-violation conflicts surface cleanly. The route lives at `/compete/organizer/{competitionId}/ai-judge-scheduler?event={trackWorkoutId}`, with a "Suggest with AI" button on the existing volunteers schedule tab linking through. Note: `routeTree.gen.ts` no longer emits the `declare module "@tanstack/react-start" { interface Register }` block in router-plugin 1.144; that block now lives at the bottom of [[apps/wodsmith-start/src/router.tsx]] so loader / search-param type inference survives future routeTree regenerations.
