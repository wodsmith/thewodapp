...
[[apps/wodsmith-start/src/server-fns/judge-scheduling-fns.ts#getJudgesScheduleDataFn]] batches active assignments, heat lanes, team registrations, complete active athlete-team rosters, base workout briefs, and per-division workout descriptions. Team registrations use their active athlete memberships as the roster and fall back to the registration owner only when no athlete memberships exist, avoiding coach or manager names on judge sheets.
