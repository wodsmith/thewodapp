...
Scores cleared by acceptance are scoped to the source user, the transferred registration's division (explicit null when absent), and the competition's track-workout IDs. The score column named `competitionEventId` stores track-workout IDs, not competition-event IDs. Acceptance locks the matching score IDs and deletes their round rows before the parent scores in the same transaction. Before deletion, it rejects any exact score tuple that may belong to another competition’s current or recorded historical participation, rolling back the entire acceptance. See [[registration#Division Transfer#Concurrent submissions]]. Retained registrations' scores and rounds are preserved; this does not introduce score history.

Transfer waiver acknowledgements reuse `waiver_signatures`, with an additive nullable `signatureName` column for the exact typed text. Legacy signing flows leave it null. The authenticated accepting user, registration and signing time identify the latest acknowledgement for the unique waiver/user pair; re-signing updates that recipient record, not the source athlete's record, and preserves its existing IP provenance when the acceptance request supplies no replacement IP. The server rejects foreign waivers, duplicate waiver IDs, missing required athlete waivers, blank names and names longer than 255 characters before registration writes. These records capture what was entered without assigning new legal meaning to that text.

Public user-status and authenticated registration-status reads in both apps exclude signature text, since they do not authorize access to that audit detail. The additive migration must be deployed before application code; no historical names are fabricated or backfilled. See [[transfer-integrity-tests#Transfer integrity]] for database and form regressions.
