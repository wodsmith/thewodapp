...
Scores cleared by acceptance are scoped to the source user, the transferred registration's division (explicit null when absent), and the competition's track-workout IDs. The score column named `competitionEventId` stores track-workout IDs, not competition-event IDs. Retained registrations' results are preserved; this does not introduce score history.

Transfer waiver acknowledgements reuse `waiver_signatures`, with an additive nullable `signatureName` column for the exact typed text. Legacy signing flows leave it null. The authenticated accepting user, registration and signing time identify the latest acknowledgement for the unique waiver/user pair; re-signing updates that recipient record, not the source athlete's record. The server rejects foreign waivers, duplicate waiver IDs, missing required athlete waivers, blank names and names longer than 255 characters before registration writes. These records capture what was entered without assigning new legal meaning to that text.
