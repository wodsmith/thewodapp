...
[[apps/crew/src/routes/events/$eventId/heats.tsx]] renders a per-workout breakdown of scheduled heats plus an "Import from CSV or Excel" modal. The loader calls [[apps/crew/src/server-fns/crew-heats-fns.ts#getCrewHeatsPageFn]] which fetches track workouts, existing heats enriched with location and division details, and location options carrying lane count, default heat gap, and default status. Access is gated by `requireCrewEventManagerAccess`.

Heat creation is bulk-first (see [[crew#Bulk Heat Scheduling]]): "Add heats" expands responsive controls and editable preview rows inside the workout card instead of opening a modal. Existing rows remain visible for schedule context. The explicit per-heat rows are persisted by `generateHeatsFn` from [[apps/crew/src/server-fns/crew-heats-fns.ts]]. Deletion uses `deleteHeatFn` with an inline confirm step. Each saved row shows its number, scheduled time, location and lane count, division, and publish status. After a mutation the page calls `router.invalidate()`.
...
The inline builder on [[apps/crew/src/routes/events/$eventId/heats.tsx]] has global controls for count, start, heat length, heat gap, and location plus one `datetime-local` input per preview heat. Its responsive grid gives the date/time and location controls wider columns so their complete values stay visible, and moves location to its own row at intermediate widths. It preselects the event's default location and adopts that location's saved gap. For a workout without heats, the start defaults to midnight on the event start date. With existing heats, [[apps/crew/src/lib/crew/heat-scheduling.ts#getInitialHeatSchedule]] continues after the highest-numbered heat using that heat's duration plus the selected location's gap; an unscheduled final heat falls back to the event start date.

[[apps/crew/src/lib/crew/heat-scheduling.ts#buildCascadedLocalTimes]] cascades the preview: heat N = `start + (N-1) × (length + gap)`, the same formula wodsmith-start's `copyHeatsFromEventFn` uses. It operates on naive wall-clock `datetime-local` strings so it never crosses a browser DST boundary; the event timezone is applied only when values become stored UTC datetimes.
...
Deleting a location does not delete its heats: [[apps/crew/src/server-fns/crew-locations-fns.ts#deleteCrewLocationFn]] nulls their `venueId`, deletes the location, and promotes a replacement default in one transaction. Affected heats keep their number and time. Location preferences flow through [[apps/crew/src/server-fns/crew-heats-fns.ts#getCrewHeatsPageFn]] so the Heats page (see [[crew#Heats Page]]) can choose low-edit defaults.
...
All create, update, delete, assign, and remove actions reuse Crew's existing event-scoped server functions in [[apps/crew/src/server-fns/crew-roster-shift-fns.ts]] (eventId plus `YYYY-MM-DD`/`HH:mm` strings normalized server-side in the event timezone), so the page shares the same auth and capacity rules as the assignments shift board. Role tags prioritize candidates and drive staffing warnings, but do not prevent an organizer from assigning a volunteer. The "Volunteer Shifts" sidebar item is registered in [[apps/crew/src/lib/crew/navigation.ts]] and allow-listed in [[apps/crew/src/components/crew-event-sidebar.tsx]].
...
The Crew judging UI ports wodsmith-start's two surfaces: a Gantt-style **Rotation Timeline** for pre-publish planning, and a heat-lane **Published Assignments** grid for post-publish fine-tuning.

[[apps/crew/src/routes/events/$eventId/judges.tsx]] renders a single page heading and then [[apps/crew/src/routes/events/$eventId/-components/judges/judge-scheduling-container.tsx]], which owns the layout (workout selector, the gated Published Assignments section on top, the Rotations section below) and the local cell state. The Published Assignments section appears only once the workout has an active published version; the Rotations section is always shown for a workout with heats.

The pre-publish surface is [[apps/crew/src/routes/events/$eventId/-components/judges/rotation-timeline.tsx]] — a coverage grid of heats × lanes where organizers click a cell or "Add" to plan a judge's rotation across heats. Crew heat numbers are not guaranteed to start at 1 (the Heats setup UI has a global starting-heat control, see [[crew#Heats Page]]) and may have gaps, so the timeline keeps two spaces: the grid geometry runs in 1-based **display-index** space while stored rotations and the editor form fields (`startingHeat`) run in **actual heat-number** space. The timeline's `displayToHeatNumber` / `heatNumberToDisplay` maps convert at every boundary — cell clicks and rotation drops convert display→actual before reaching the editor, editor preview cells convert actual→display before matching grid keys, and column headers render actual heat numbers. The editor validates/clamps against the workout's real min/max heat numbers (`minHeat`/`maxHeat`), not a heat count — its zod schema is built per-instance so `startingHeat` is bounded to `[minHeat, maxHeat]`, `startingLane` to the max lane count, and `heatsCount` to the heat range, preventing saves that would expand to zero cells. Heats can also have differing lane counts: coverage is always computed from each heat's own `laneCount` (so the timeline's coverage % matches the RotationOverview), grid cells whose lane exceeds a heat's own lane count render as unavailable (assignments there would be dropped by `expandRotationToAssignments`), and the editor's live preview mirrors `expandRotationToAssignments` exactly — skipping heat-number gaps and cycling shift-right lanes per-heat — via a `heats` prop threaded from the timeline (memoized as `editorHeats` — the editor feeds preview cells back through `onPreviewChange` → `setPreviewCells`, so an inline-built array here closes an infinite re-render loop). [[apps/crew/src/routes/events/$eventId/-components/judges/multi-rotation-editor.tsx]] is the per-judge rotation form (multiple collapsible rotation blocks with live preview); its judge dropdown shows each judge's rotation count for the current workout inline with their name, and a compact two-line "Last:" hint (ending heat number with its scheduled time, then which event it's from on the next line) computed across ALL of the event's workouts — the container derives this `latestAssignmentByJudge` map from the full rotation set, mirroring `expandRotationToAssignments` gap-skipping to find each rotation's true ending heat, ordered by workout trackOrder then ending heat. And [[apps/crew/src/routes/events/$eventId/-components/judges/rotation-overview.tsx]] shows coverage metrics plus the [[apps/crew/src/routes/events/$eventId/-components/judges/publish-rotations-button.tsx|Publish]] control. An "Event defaults" tray ([[apps/crew/src/routes/events/$eventId/-components/judges/event-defaults-editor.tsx]]) sets per-workout rotation defaults — default rotation length, stay-in-lane vs shift-right, and the minimum heat buffer — persisted via `updateCrewJudgeEventDefaultsFn` (scoped through Crew event manager access, not the team-membership permission path); its number fields autosave debounced (raw string state so clearing/retyping works, invalid values snap back on blur) with no success toast; typing an invalid value cancels any pending debounced save so a superseded value never fires, and prop-syncs are skipped while a field is focused so a refresh can't clobber mid-typing input. The timeline deliberately preserves its UI state (open editor, selection, expanded judges) across post-mutation loader refreshes — the container remounts it per workout via `key`, so only stale rotation-id selections are cleared when saves replace rotation rows. Coverage and expansion reuse [[apps/crew/src/lib/judge-rotation-utils.ts]] (`calculateCoverage`, `expandRotationToAssignments`, `filterRotationsByAvailability`), resolving each rotation to the same membership-or-invitation canonical assignee id, so multi-heat rotations with lane-shift patterns and buffer zones render exactly as in wodsmith-start.

The post-publish surface keeps the per-cell heat-lane grid: [[apps/crew/src/routes/events/$eventId/-components/judges/judge-heat-card.tsx]] and [[apps/crew/src/routes/events/$eventId/-components/judges/draggable-judge.tsx]] provide the `@atlaskit/pragmatic-drag-and-drop` draggable judges and droppable lanes. Each grid cell (one judge in one heat + lane) is a single-heat rotation (`heatsCount = 1`, lane-shift `stay`); [[apps/crew/src/routes/events/$eventId/-components/judges/judge-grid-utils.ts]] converts saved rotations to grid cells and back ([[apps/crew/src/routes/events/$eventId/-components/judges/judge-grid-utils.ts#rotationsToGridCells]], [[apps/crew/src/routes/events/$eventId/-components/judges/judge-grid-utils.ts#gridCellsToRotationRows]]).

Both surfaces persist rotations through the same canonical-id-aware path: every save (timeline editor, cell drop/move/remove, or rotation drag) replaces the affected judge's full rotation set via `saveCrewJudgeRotationsForVolunteerFn`, and the Publish button materializes them with `publishCrewJudgeRotationsFn`. Only the per-workout defaults tray adds a server function (`updateCrewJudgeEventDefaultsFn`).

Draft `competition_judge_rotations` stay editable after publishing — saves are never blocked by an existing published version. A published version is an immutable snapshot keyed by its `versionId`; its `judge_heat_assignments` rows persist independently and carry `rotationId` only as provenance (a plain column, not a DB foreign key, and unused for active-assignment hydration). So adjusting the grid or timeline after publishing replaces the draft rotations, and re-publishing materializes the updated draft into a brand-new active version while prior versions stay intact for audit and rollback ([[crew#Judge Assignment Version Publishing]]).
...
[[apps/crew/src/routes/e/$slug/schedule/index.tsx]] reads only the stored snapshot, rechecks the event slug, Crew-only status, lifecycle, and current access, and uses no-store/noindex responses. Renaming a Crew event updates that slug; republishing stores it in the snapshot, and both copy actions then use the renamed event's URL. [[apps/crew/src/components/crew/public-schedule-view.tsx]] provides name lookup, separate choices for duplicate names, event-timezone assignments, and a printable personal schedule. Anyone with the link can browse names and assignments; this is public event sharing rather than identity verification. Unpublishing or losing paid access makes the link unavailable.
...
The tabs are **Master Schedule** (shifts and heats combined, day-sectioned, with a Master CSV download), **Judges** (per event/workout, a clear heat × lane × judge table), and **Shifts** (each shift as its own section listing who is on and when). Printable judge and shift tables deliberately omit volunteer confirmation/check-in status.
