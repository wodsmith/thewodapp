...
[[packages/wodsmith-db/src/schemas/crew-published-schedules.ts]] stores one current JSON snapshot per competition. The additive shared migration is `packages/wodsmith-db/mysql-migrations/0012_crew_published_schedules.sql`; both apps re-export the schema. Publishing atomically replaces that row, and unpublishing removes it. Live staffing reads are not wrapped in one shared read transaction, so organizers should finish concurrent schedule editing before publishing.
