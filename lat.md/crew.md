...
The inline builder on [[apps/crew/src/routes/events/$eventId/heats.tsx]] has global controls for count, start, heat length, heat gap, and location plus one `datetime-local` input per preview heat. It preselects the event's default location and adopts that location's saved gap. For a workout without heats, the start defaults to midnight on the event start date. With existing heats, [[apps/crew/src/lib/crew/heat-scheduling.ts#getInitialHeatSchedule]] continues after the highest-numbered heat using that heat's duration plus the selected location's gap; an unscheduled final heat falls back to the event start date.
