...
Personal, organizer, athlete, video, and submission-review score writes share one in-app workout-result module while adapters retain authentication, ownership, policy, lifecycle, logging, and response contracts.
...
[[apps/wodsmith-start/src/server/workout-results/mobile-video.ts#normalizeMobileVideoWorkoutResult]] and [[apps/wodsmith-start/src/server/workout-results/mobile-video.ts#persistMobileVideoWorkoutResult]] form the mobile video HTTP interface used by [[apps/wodsmith-start/src/routes/api/compete/video/submit.ts]]. The route retains auth, registration and window checks, video lifecycle, and response serialization while the interface preserves explicit below-threshold CAP, null invalid tiebreaks, and the video-first non-transactional score write.

[[apps/wodsmith-start/src/server/workout-results/judge.ts#normalizeJudgeWorkoutResult]] and [[apps/wodsmith-start/src/server/workout-results/judge.ts#persistJudgeWorkoutResult]] form the judge HTTP interface used by [[apps/wodsmith-start/src/routes/api/compete/scores/judge.ts]]. The route retains supplied-team authorization, client-workout fallback, window policy, logging, and response shapes. Its normalizer delegates competition scoring to [[apps/wodsmith-start/src/server/workout-results/normalize.ts#normalizeCompetitionWorkoutResult]] with only a judge-specific invalid-round error adapter; persistence keeps exact nullable-division readback and atomic score/round replacement through the shared round writer.

[[apps/wodsmith-start/src/server/workout-results/self-entry.ts#normalizeAthleteSelfEntryWorkoutResult]] and [[apps/wodsmith-start/src/server/workout-results/self-entry.ts#persistAthleteSelfEntryWorkoutResult]] form the athlete self-entry interface used by [[apps/wodsmith-start/src/server-fns/athlete-score-fns.ts#submitAthleteScoreFn]]. It preserves explicit CAP clamping, null invalid-tiebreak persistence, and the non-transactional upsert-before-exact-division-read order. Valid CAP secondary reps and tiebreak values participate in the persisted sort key.

[[apps/wodsmith-start/src/server/workout-results/review.ts#normalizeSubmissionScoreAdjustment]], [[apps/wodsmith-start/src/server/workout-results/review.ts#updateReviewedSubmissionWorkoutResult]], [[apps/wodsmith-start/src/server/workout-results/review.ts#normalizeManualSubmissionWorkoutResult]], and [[apps/wodsmith-start/src/server/workout-results/review.ts#insertManualSubmissionWorkoutResult]] form the submission-review interfaces. [[apps/wodsmith-start/src/server-fns/submission-verification-fns.ts#verifySubmissionScoreFn]] keeps reviewer identity, audit and video lifecycle outside the module while passing its transaction to the score/round writer; direct overrides retain stale rounds and their CAP count, while round replacements must restate the complete contiguous breakdown. Invalidated results retain a worst-place sort key, and malformed manual scores are rejected. [[apps/wodsmith-start/src/server-fns/submission-verification-fns.ts#enterSubmissionScoreFn]] likewise keeps existence checks, duplicate mapping, audit, and video review policy in the adapter while the score insert, exact-division read, and optional round writes remain inside the adapter-owned transaction.
...
The focused boundaries in [[tests/workout-result-adapters#Workout-result Adapter Characterization]] retain deliberate compatibility differences: self-entry trusts explicit CAP and stores null invalid tiebreaks; mobile video preserves explicit below-threshold CAP and video-first writes; judge entry trusts supplied workout/athlete targets and can retain stale rounds; verification direct overrides retain stale rounds without clamping; manual entry trusts explicit CAP and couples score persistence to its audit/video transaction.

Mobile [[apps/wodsmith-start/src/routes/api/compete/scores/submit.ts]] remains adapter-local. Its null-division score-id readback omits the division predicate, unlike the exact-null interfaces, so extraction is deferred until the compatibility and security behavior is explicitly chosen. Crew still owns copied implementations; shared packaging starts after that decision by moving the context-neutral kernel/round writer plus these narrow interfaces behind database-agnostic contracts without moving auth, policy, lifecycle, logging, or HTTP serialization.

### One score per athlete per event per division

The `scoresTable` unique index spans `(competition_event_id, user_id, scaling_key)` so an athlete registered in multiple divisions can hold a separate score per division on a shared workout.

`scalingKey` is a stored generated column defined as `coalesce(scaling_level_id, '')`. It preserves each real division id while mapping a null/open division to a stable empty key, making competition upserts null-safe under MySQL's otherwise-distinct NULL semantics.

Column-name note: `scores.competition_event_id` is misleadingly named — it actually stores a `track_workouts.id` (the value the caller supplies as `trackWorkoutId`), not a `competition_events.id`. Every insert path assigns it from `data.trackWorkoutId`. That scope is exactly right for this invariant: when the same workout appears in multiple divisions via event-division mappings, both divisions reference the same `track_workout` row, so the unique key `(track_workout, user, scaling_key)` is what lets a multi-division athlete carry one score per division. The same naming gotcha bites delete paths: keying a score delete off `competitionEventsTable.id` matches nothing because that column holds the competition-event row id, not a `track_workouts.id` — score deletes must select `competitionEventsTable.trackWorkoutId` and compare to `scoresTable.competitionEventId`.

Online competitions allow the same track workout to appear in multiple divisions (e.g. scheduled for both Individual and Partner), and athletes can register in more than one division. Keying only on `(track_workout, user)` would merge an athlete's individual and partner scores into one row via `onDuplicateKeyUpdate` — the last submitted score would overwrite the other, and every leaderboard/athlete-detail read (which joins on `track_workout + user`) would surface that single row against every registration the user owned, leaking partner scores onto the individual leaderboard and vice versa. Every score-write path ([[apps/wodsmith-start/src/server-fns/video-submission-fns.ts#submitVideoFn]], [[apps/wodsmith-start/src/server-fns/competition-score-fns.ts#saveCompetitionScoreFn]], [[apps/wodsmith-start/src/server-fns/athlete-score-fns.ts#submitAthleteScoreFn]], [[apps/wodsmith-start/src/server-fns/submission-verification-fns.ts]], [[apps/wodsmith-start/src/routes/api/compete/scores/submit.ts]], [[apps/wodsmith-start/src/routes/api/compete/scores/judge.ts]], [[apps/wodsmith-start/src/routes/api/compete/video/submit.ts]]) scopes the registration lookup by divisionId and carries that divisionId through to the upsert's `scalingLevelId`. Submission fns that accept an athlete without a divisionId reject with an ambiguity error when the athlete is registered in more than one division rather than silently picking one. Every score-read path ([[apps/wodsmith-start/src/server/competition-leaderboard.ts#getCompetitionLeaderboard]], [[apps/wodsmith-start/src/server/series-leaderboard.ts]], [[apps/wodsmith-start/src/server-fns/organizer-athlete-fns.ts#getOrganizerAthleteDetailFn]], [[apps/wodsmith-start/src/server-fns/division-results-fns.ts]]) scopes scores by `scaling_level_id` and groups/attributes by the score's own `scalingLevelId` rather than by the first registration found for that userId — the leaderboard maps each score to the exact `(user, division)` registration, not just userId. Delete paths ([[apps/wodsmith-start/src/server-fns/competition-score-fns.ts#deleteCompetitionScoreFn]], [[apps/wodsmith-start/src/server-fns/registration-fns.ts#removeRegistrationFn]]) scope by division so removing one registration or score doesn't nuke the athlete's scores in their other division on a shared workout. The registration-transfer path in [[apps/wodsmith-start/src/server/commerce/transfer-handlers.ts]] intentionally deletes all the source user's scores across divisions because the user is being fully transferred out of the competition.

#### Null divisionId is its own scope

Null `scalingLevelId` (open / no-division registration) is queried with `isNull(scoresTable.scalingLevelId)`, never an absent predicate.

Persisted uniqueness is enforced separately by the generated non-null `scalingKey`; dropping the read predicate would still leak scores across divisions on a shared workout.

The canonical predicate, repeated across every score read/write/delete path, is `divisionId ? eq(scalingLevelId, divisionId) : isNull(scalingLevelId)`.

[[apps/wodsmith-start/src/server-fns/competition-score-fns.ts#deleteCompetitionScoreFn]] takes `divisionId: z.string().nullable()` — required-but-nullable — so callers must explicitly pick a scope. Optional was unsafe: a missing field collapsed to an unscoped delete that wiped every division's score for the user/workout pair. [[apps/wodsmith-start/src/server-fns/athlete-score-fns.ts#getAthleteEventScoreFn]] uses `resolveAthleteDivisionId`, which returns a `{ kind: "specific" | "open" | "ambiguous" | "notFound" }` discriminated union; ambiguous and not-found cases short-circuit to an empty score rather than dropping the divisionId predicate (which would leak across divisions). [[apps/wodsmith-start/src/server-fns/competition-score-fns.ts#getEventScoreEntryDataFn]] keys its score map by `(userId, scalingLevelId)` rather than `userId` alone, so a multi-division athlete's division-A score never gets attached to their division-B registration row.
...
Benchmark batteries define reusable threshold tables for generic benchmark competitions.

[[packages/wodsmith-db/src/schemas/benchmarks.ts#benchmarkBatteriesTable]] stores the battery metadata, category cache, rating bands, scoring scale, video policy, optional open-join flag, variant scaling group, and linked competition. [[packages/wodsmith-db/src/schemas/benchmarks.ts#benchmarkTestsTable]] stores the ordered benchmark tests inside a battery, including category, score scheme, score type, input unit, included/deferred state, and the hybrid score-model metadata (`scoreModel`, `hybridFlipTier`, `timeCapMs`). [[packages/wodsmith-db/src/schemas/benchmarks.ts#benchmarkTierThresholdsTable]] stores one encoded threshold per test, variant, and tier while preserving the source display value for extraction receipts.

### Hybrid reps/time tests

Time-capped tests where most athletes won't finish (e.g. Open 16.2, Open 18.4) use `scoreModel="hybrid"` with a `hybridFlipTier`: tier thresholds below the flip are reps completed at the cap, thresholds at or above it are finish times.

Hybrid tests must use the `time-with-cap` scheme and flip between tier 2 and the battery's `maxTier` so both dimensions keep at least one threshold — [[apps/wodsmith-start/src/server/benchmark-scoring-tiers.ts#resolveBenchmarkTestScoreModel]] enforces this on test create/update and [[apps/wodsmith-start/src/server/benchmark-leaderboard.ts#resolveHybridFlipTier]] on leaderboard context loads.

Thresholds encode per dimension: [[apps/wodsmith-start/src/server/benchmark-scoring-tiers.ts#benchmarkThresholdDimension]] maps a tier to `"cap_reps"` or `"score"`, and [[apps/wodsmith-start/src/server/benchmark-scoring-tiers.ts#encodeBenchmarkThresholdValue]] stores rep tiers as plain integers (rejecting non-integer input like `19:59` so a time can't silently truncate to 19 reps) and time tiers as milliseconds. Tier evaluation in [[apps/wodsmith-start/src/lib/scoring/algorithms/absolute-tier.ts#calculateAbsoluteTier]] reads the score's status: a finisher (`status="scored"`) clears every rep tier outright — finishing means all reps were completed — plus any time tier its finish time beats, while a capped athlete (`status="cap"`, cap ms in `scoreValue`, reps in `secondaryValue`) is measured only against rep tiers via `EventScoreInput.secondaryValue`. Capped scores that tie on tier and cap value compare by reps completed when deciding whether a resubmission improves an athlete's best.

The organizer tier editor labels each cell's dimension (reps vs `mm:ss` placeholders, plus a per-test flip summary) in [[apps/wodsmith-start/src/routes/compete/organizer/$competitionId/scoring/tiers.tsx]], and the test dialogs in [[apps/wodsmith-start/src/components/benchmark-tiers/test-editor-dialogs.tsx]] expose the score model and flip tier, forcing the `time-with-cap` scheme when hybrid is selected.

Benchmark competitions reuse the normal competition shell. [[packages/wodsmith-db/src/schemas/programming.ts#trackWorkoutsTable]] carries nullable `benchmarkTestId` and `benchmarkCategory` columns so leaderboard reads can bind each competition event to its source test and category. [[packages/wodsmith-db/src/schemas/scores.ts#scoresTable]] carries nullable `benchmarkVariant` so each score snapshots the athlete's scoring variant at submission time.

Benchmark configuration validates before write/read code consumes it. [[apps/wodsmith-start/src/schemas/benchmark.schema.ts#benchmarkCategoriesSchema]] rejects duplicate or malformed category caches, [[apps/wodsmith-start/src/schemas/benchmark.schema.ts#getBenchmarkCategoryCountIssues]] compares cached `testCount` values to included tests, [[apps/wodsmith-start/src/schemas/benchmark.schema.ts#benchmarkVariantSchema]] limits v1 score variants to the profile gender values, and [[apps/wodsmith-start/src/schemas/benchmark.schema.ts#benchmarkThresholdValuesSchema]] requires ten tier thresholds per standard row. [[apps/wodsmith-start/src/lib/scoring/algorithms/absolute-tier.ts#calculateAbsoluteTier]] fails closed on missing variant/table data, and [[apps/wodsmith-start/src/lib/scoring/category-aggregation.ts#aggregateBenchmarkScores]] computes category scores and Overall/100 without a second rescale.

Benchmark leaderboard reads use the same public leaderboard path and always rank with the online algorithm — forced at read time by [[apps/wodsmith-start/src/server/competition-leaderboard.ts#resolveLeaderboardScoringConfig]] — while tier data renders as additive context (see [[competition-type-capabilities#Additive Tier Context]]). [[apps/wodsmith-start/src/server/benchmark-leaderboard.ts#loadBenchmarkLeaderboardContext]] loads and validates battery categories, one-to-one test/event mappings, variants, and threshold completeness once per request for any competition granting `benchmarkScoringTiers`, degrading to no-context on incomplete setup instead of failing the board. [[apps/wodsmith-start/src/server/competition-leaderboard.ts#getCompetitionLeaderboard]] computes per-event tiers from the threshold tables, attaches Overall/100, rating, category, tier, inclusion, and review metadata, and ignores stale publish-gate rows for perpetual benchmark boards. [[apps/wodsmith-start/src/components/online-competition-leaderboard-table.tsx#OnlineCompetitionLeaderboardTable]] renders benchmark boards with Overall/100, rating, category summaries, per-test tiers, and generic verification labels whenever entries carry tier data, while [[apps/wodsmith-start/src/components/benchmark-stat-line.tsx#BenchmarkStatLine]] and [[apps/wodsmith-start/src/routes/compete/$slug/stats.tsx#BenchmarkStatsPage]] expose a generic stats tab for benchmark competitions. On the stats tab, test-grid rows expand into an inline [[apps/wodsmith-start/src/components/compete/video-submission-form.tsx#VideoSubmissionForm]] only for the viewer's own entry; rows whose test is deferred (`benchmarkIncludedInScoring=false`, the "Unavailable" state) stay read-only because [[apps/wodsmith-start/src/server/benchmark-submissions.ts]] rejects scored submissions for deferred tests.

Benchmark score writes are centralized behind the video-submission path. [[apps/wodsmith-start/src/server-fns/video-submission-fns.ts#submitVideoFn]] detects benchmark batteries and delegates to [[apps/wodsmith-start/src/server/benchmark-submissions.ts#saveBenchmarkScoreInTransaction]], which requires profile gender via [[apps/wodsmith-start/src/server/benchmark-submissions.ts#getBenchmarkProfileVariant]], writes `benchmarkVariant`, stores scores under the single Open division, keeps the best score by absolute tier plus raw-score direction, and lets accepted score, video review reset, and round rewrites commit together. Benchmark athletes must use the normal registration flow before scoring; unregistered athletes receive the standard registration-required state from [[apps/wodsmith-start/src/server-fns/video-submission-fns.ts#getVideoSubmissionFn]] and [[apps/wodsmith-start/src/server-fns/video-submission-fns.ts#getBatchEventVideoSubmissionsFn]] instead of being auto-joined during score submission.

The first benchmark seed treats the source PDF as provenance data, not a branded product surface. [[apps/wodsmith-start/scripts/seed/data/benchmark-training-guide.ts#buildBenchmarkSeedRows]] creates generic competition, battery, workout, and track-workout rows from 58 source tests — all 58 included in scoring now that hybrid reps/time scoring exists — and leaves `competition_events` empty so the board stays perpetual without submission windows. The two hybrid tests (Open 16.2, flip tier 9 with a 20:00 cap; Open 18.4, flip tier 7 with a 9:00 cap) carry `timeCapSeconds` so their workout rows get `time_cap` and their test rows get `time_cap_ms`, and their rep-tier thresholds encode as plain integers while time tiers encode as milliseconds. Weighted C2B Pull Up is scored as added load in pounds: the seed encodes its "BW" threshold as 0 lb and "+N" thresholds as N lb, and the workout description tells athletes to enter 0 for a bodyweight rep. The seed starts with `scoringConfig.algorithm="online"`, the only ranking mode for benchmark boards; tiers are additive display context. The seed keeps `isOpenJoin=false`, so registration remains explicit even though the board has no close date or submission windows. Every benchmark track workout is seeded with `event_status="published"` so the benchmark leaderboard read still satisfies [[apps/wodsmith-start/src/server/benchmark-leaderboard.ts#loadBenchmarkLeaderboardContext]]'s one-to-one test↔track-workout mapping. Customer-facing routes, navigation, stats pages, copy, logos, calls to action, and visual treatment remain generic WODsmith benchmark surfaces.

The seed also populates the board with ten demo athletes so the leaderboard and stats surfaces render with data. [[apps/wodsmith-start/scripts/seed/data/benchmark-training-guide.ts#buildBenchmarkAthleteSeedRows]] takes the five male and five female athletes in [[apps/wodsmith-start/scripts/seed/data/benchmark-training-guide.ts#BENCHMARK_SEED_ATHLETES]] (users seeded by 03-users), gives each a competition-team membership and an Open-division registration, and writes one score per athlete per included test. Each athlete has a base tier plus per-category bias, and [[apps/wodsmith-start/scripts/seed/data/benchmark-training-guide.ts#benchmarkTierFor]] adds a deterministic -2..+2 per-test jitter; score values sit exactly on the resulting tier's threshold for the athlete's variant, so achieved tiers are known by construction. On hybrid tests, an athlete landing on a rep tier is written as a capped score (`status="cap"`, cap ms in `score_value`, reps in `secondary_value`) while a time-tier athlete is written as a finisher. Every score row snapshots `benchmarkVariant` the way [[apps/wodsmith-start/src/server/benchmark-submissions.ts#saveBenchmarkScoreInTransaction]] would at submission time.
...
The form mirrors the athlete-facing `video-submission-form.tsx` score section — schema-aware `parseScore` validation, single vs multi-round inputs, auto-derived cap status from parsed time vs `timeCap` (no separate status picker), secondary "reps at cap" when the cap is hit, and a tiebreak input when `workout.tiebreakScheme` is set — plus reviewer-specific `reviewerNotes` and optional `noRepCount`. Tiebreak input is validated with `parseScore(tiebreakScheme)` before submit so invalid values surface an inline error instead of being silently dropped by the server's try/catch-around-encode. POSTs to [[apps/wodsmith-start/src/server-fns/submission-verification-fns.ts#enterSubmissionScoreFn]]. The server fn is INSERT-only — it loads the video submission + registration + workout, refuses to overwrite an existing `scoresTable` row for the same `(userId, competitionEventId)`, then calls [[apps/wodsmith-start/src/server/workout-results/review.ts#normalizeManualSubmissionWorkoutResult]] and [[apps/wodsmith-start/src/server/workout-results/review.ts#insertManualSubmissionWorkoutResult]] inside its audit/video transaction. The module interface writes the new `scores` row and per-round `score_rounds` rows when applicable; the adapter writes the audit entry (action `"adjusted"` with null `originalScoreValue`) and flips the video submission to `reviewStatus = "adjusted"`. Score ownership follows the same captain-only convention as `submitVideoFn`: `scoresTable.userId` is the registration's `captainUserId ?? userId`, and `teamId` is the workout's track owner.
