...
Benchmark leaderboard reads use the same public leaderboard path with a preloaded benchmark context. [[apps/wodsmith-start/src/server/benchmark-leaderboard.ts#loadBenchmarkLeaderboardContext]] validates battery categories, one-to-one test/event mappings, variants, and threshold completeness once per request, then [[apps/wodsmith-start/src/server/competition-leaderboard.ts#getCompetitionLeaderboard]] uses absolute tiers as exact event points, replaces total points with Overall/100 from `aggregateBenchmarkScores`, ignores stale publish-gate rows for perpetual benchmark boards, and attaches rating, category, tier, inclusion, and review metadata for the generic table and stat-line UI. [[apps/wodsmith-start/src/components/online-competition-leaderboard-table.tsx#OnlineCompetitionLeaderboardTable]] renders benchmark boards with Overall/100, rating, category summaries, per-test tiers, and generic verification labels, while [[apps/wodsmith-start/src/components/benchmark-stat-line.tsx#BenchmarkStatLine]] and [[apps/wodsmith-start/src/routes/compete/$slug/stats.tsx#BenchmarkStatsPage]] expose a generic stats tab only for `absolute_tier` boards.
