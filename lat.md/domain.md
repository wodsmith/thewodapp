...
Benchmark leaderboard reads use the same public leaderboard path and stay powered by the configured scoring algorithm. When `scoringConfig.algorithm` is `online`, [[apps/wodsmith-start/src/server/competition-leaderboard.ts#getCompetitionLeaderboard]] ranks the board with the regular online scoring algorithm and does not preload absolute-tier context. When an organizer switches the benchmark to `absolute_tier`, [[apps/wodsmith-start/src/server/benchmark-leaderboard.ts#loadBenchmarkLeaderboardContext]] validates battery categories, one-to-one test/event mappings, variants, and threshold completeness once per request; the leaderboard then uses absolute tiers as exact event points, replaces total points with Overall/100 from `aggregateBenchmarkScores`, ignores stale publish-gate rows for perpetual benchmark boards, and attaches rating, category, tier, inclusion, and review metadata for the generic table and stat-line UI. [[apps/wodsmith-start/src/components/online-competition-leaderboard-table.tsx#OnlineCompetitionLeaderboardTable]] renders absolute-tier benchmark boards with Overall/100, rating, category summaries, per-test tiers, and generic verification labels, while [[apps/wodsmith-start/src/components/benchmark-stat-line.tsx#BenchmarkStatLine]] and [[apps/wodsmith-start/src/routes/compete/$slug/stats.tsx#BenchmarkStatsPage]] expose a generic stats tab only for `absolute_tier` boards. On the stats tab, test-grid rows expand into an inline [[apps/wodsmith-start/src/components/compete/video-submission-form.tsx#VideoSubmissionForm]] only for the viewer's own entry; rows whose test is deferred (`benchmarkIncludedInScoring=false`, the "Unavailable" state) stay read-only because [[apps/wodsmith-start/src/server/benchmark-submissions.ts]] rejects scored submissions for deferred tests.
