...
Scores saved before this fix have `scoreValue` clamped to the cap even though rounds were persisted correctly. [[apps/wodsmith-start/src/server-fns/competition-score-fns.ts#backfillMultiRoundCapScoresFn]] rescans a competition, re-aggregates parent `scoreValue` from rounds using the workout's `scoreType`, and recomputes per-round status, parent status, and `sortKey`. Supports a `dryRun` flag to preview changes before writing. If the workout is missing `scoreType`, the result reports whether it fell back to the score row or the default `time-with-cap` score type and includes per-score warnings.

On leaderboards and anywhere else scores flow through [[apps/wodsmith-start/src/lib/scoring/format/score.ts#formatScore]], a multi-round capped score renders the aggregate time from the parent score (the sum of completed round times and capped-round time caps). Per-round rows show `CAP (time cap, reps)` for capped rounds so viewers can see which rounds capped without pretending the entire event score was a single 18:00 cap. Single-round capped scores still render as `CAP (N reps)` because they carry `timeCap.secondaryValue`. Sort order is unaffected — capped scores always sort below fully scored entries via `STATUS_ORDER`, regardless of how large the summed total is, so a 50:00 fully-scored run still outranks a 41:00 capped run.

The public leaderboard also surfaces per-round breakdowns so viewers can distinguish "capped one round of two" from "capped every round" and see how the aggregate was built. [[apps/wodsmith-start/src/server/competition-leaderboard.ts#getCompetitionLeaderboard]] fetches `scoreRoundsTable` for every score on the page via `fetchRoundCapSummaries` and attaches `{cappedRoundCount, totalRoundCount, rounds}` to each event result, with each round formatted from its persisted value and status. The `CappedRoundsIndicator` and round-breakdown renderers in [[apps/wodsmith-start/src/components/competition-leaderboard-table.tsx]] and [[apps/wodsmith-start/src/components/online-competition-leaderboard-table.tsx]] render an amber `N/M CAP` badge next to `formattedScore` whenever `cappedRoundCount > 0 && totalRoundCount > 1`, then list each round under the aggregate. Single-round scores render unchanged.

The `22-mwfc-round-breakdown` seed in [[apps/wodsmith-start/scripts/seed/seeders/22-mwfc-round-breakdown.ts#seed]] adds a MWFC-derived team division/event to the admin-owned demo `Online Qualifier 2026` competition. Its ranks, cap counts, teams, and video URLs come from the public leaderboard snapshot; its two `score_rounds` rows per score are reconstructed because the public payload did not expose authoritative round rows. The seeder refreshes its own event rows on rerun and stores ADR-0014-style round truth: each round has explicit `status`, capped rounds store the 18:00 cap as `value`, capped `secondary_value` stores reps completed at cap, and scored rounds keep `secondary_value = null`.

Within the "cap" status bucket, sorting prioritizes *how many rounds were capped* before total time. A score with 1 capped round of 2 always ranks ahead of a score with 2 capped rounds of 2, even if the 2/2-capped score has a faster summed total. When the cap count and aggregate time are equal, the summed capped-round `secondaryValue` (reps completed at cap) breaks the tie, so two teams displaying `36:00` do not tie unless their capped reps are also equal. [[apps/wodsmith-start/src/lib/scoring/sort/sort-key.ts#computeSortKey]] bit-packs `cappedRoundCount` into the top 8 bits of the 40-bit primary segment (with the low 32 bits holding the ms total, which covers ~49 days) so the database sort order naturally honors this; [[apps/wodsmith-start/src/lib/scoring/sort/compare.ts#compareScores]] mirrors the same rule for in-memory sorts via `sortScores`. To avoid relying on a stale persisted `sortKey` (stored keys are only refreshed when a score flows through `computeSortKey`, so direct DB edits or pre-fix writes leave it wrong), `getCompetitionLeaderboard` recomputes the sort key in-memory for every score it displays using the freshly-fetched round summary — no backfill required for the UI to reflect the correct order.
...
[ADR-0014](../docs/adr/0014-round-aware-multi-round-cap-scoring-followup.md) records the next design step: explicit per-round cap status plus reps-at-cap capture, while preserving the existing `cappedRoundCount` ordering. After that ADR is implemented, multi-round `time-with-cap` writes must fail validation if they omit per-round status instead of silently falling back to `encodedRound >= timeCap` inference.
...
[ADR-0014](../docs/adr/0014-round-aware-multi-round-cap-scoring-followup.md) requires public leaderboard cells to show aggregate score plus full round breakdown. The public tables now render persisted round rows under the aggregate and include `secondaryValue` for capped rounds when that value exists.
...
[ADR-0014](../docs/adr/0014-round-aware-multi-round-cap-scoring-followup.md) requires future multi-round adjustments and penalties to snapshot full round state and keep parent totals, round rows, and displays consistent. Direct multi-round adjustments without a full replacement round set must either be rejected or explicitly clear the persisted round breakdown in the same transaction.
