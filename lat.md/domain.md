...
[[adr#ADR-0014|ADR-0014]] records the next design step: explicit per-round cap status plus reps-at-cap capture, while preserving the existing `cappedRoundCount` ordering. After that ADR is implemented, multi-round `time-with-cap` writes must fail validation if they omit per-round status instead of silently falling back to `encodedRound >= timeCap` inference.
...
[[adr#ADR-0014|ADR-0014]] requires public leaderboard cells to show aggregate score plus full round breakdown. The public tables now render persisted round rows under the aggregate and include `secondaryValue` for capped rounds when that value exists.
...
[[adr#ADR-0014|ADR-0014]] requires future multi-round adjustments and penalties to snapshot full round state and keep parent totals, round rows, and displays consistent. Direct multi-round adjustments without a full replacement round set must either be rejected or explicitly clear the persisted round breakdown in the same transaction.
