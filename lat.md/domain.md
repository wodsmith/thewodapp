...
Competition writers share one command boundary, while personal training logs expose a separate domain surface.

[[apps/wodsmith-start/src/server/competition-results/domain.ts]] defines the record command, explicit division scope, claim, receipt, and stable domain errors. [[apps/wodsmith-start/src/server/competition-results/service.ts#recordCompetitionResult]] loads the workout and owning team from the programmed `track_workouts` row; caller-supplied workout metadata is never scoring authority.

[[apps/wodsmith-start/src/server/competition-results/decision.ts#decideCompetitionResult]] is the pure decision core. It validates single or complete round claims, derives the parent projection from the same claim, rejects malformed values, and follows ADR-0010: each round's explicit `status` and `secondaryScore` determine CAP state instead of comparing encoded time to the cap.

[[apps/wodsmith-start/src/server/competition-results/repository.ts#persistCompetitionResult]] atomically upserts the exact `(track workout, athlete, division)` result, re-reads that exact key, deletes every prior round, and inserts the complete new round set. A single-score overwrite therefore cannot retain stale multi-round rows, and per-round secondary values persist with their explicit CAP state.

Organizer entry, athlete self-entry, web video, judge API, and mobile video all call [[apps/wodsmith-start/src/server/competition-results/service.ts#recordCompetitionResult]]. Authentication, registration, submission windows, video lifecycle, logging, and HTTP response policy remain in their adapters. Review decisions share performance validation for complete round replacements. Parent-only adjudication preserves existing rounds and their CAP facts while overriding the aggregate; it does not re-record a single round. Review audit and video-state changes remain in their outer transaction.

[[apps/wodsmith-start/src/server/competition-results/service.ts#recordCompetitionResultInTransaction]] lets video submission commit its video and score together. The repository owns exact score identity and round replacement; pure review decisions live in `competition-results/review.ts`. Neutral scoring helpers live in `lib/scoring/result.ts`, while personal writes and their round persistence live in `training-logs/`.

Round entry uses explicit CAP controls and reps completed. The parent secondary value sums known capped-round reps and participates in ranking. Organizer and video read models return round status and secondary values so reopening an entry preserves its facts. Multi-round recording requires the complete round set; terminal outcomes may omit it.
...
The Start adapter at [[apps/wodsmith-start/src/application/scores/remove-competition-score.ts#removeCompetitionScore]] re-resolves the competition, event, and active registration inside the delete transaction. Participation belongs to either the registration owner or an active member of its athlete team; the proof retains the requested athlete so a teammate's own score is selected. The adapter then deletes round projections before exact-scope score projections. Missing score rows remain idempotent. Duplicate legacy score rows in the same proven scope are removed together; ambiguous registrations are rejected rather than chosen by row order.
...
Personal workout writes are published through [[apps/wodsmith-start/src/server/training-logs/index.ts]]. They may reuse neutral scoring helpers, but competition callers cannot import personal commands and personal callers cannot import competition commands.

Mobile [[apps/wodsmith-start/src/routes/api/compete/scores/submit.ts]] remains adapter-local. Its null-division score-id readback omits the division predicate, unlike the exact-null interfaces, so extraction is deferred until the compatibility and security behavior is explicitly chosen. Crew still owns copied implementations; shared packaging starts after that decision by moving neutral scoring helpers plus the domain interfaces behind database-agnostic contracts without moving auth, policy, lifecycle, logging, or HTTP serialization.
...
The event page retains independent child and standalone drafts when keyed child forms unmount and remount while the page stays mounted.
