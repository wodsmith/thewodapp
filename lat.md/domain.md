...
Organizer entry, athlete self-entry, web video, judge API, and mobile video all call [[apps/wodsmith-start/src/server/competition-results/service.ts#recordCompetitionResult]]. Authentication, registration, submission windows, video lifecycle, logging, and HTTP response policy remain in their adapters. Review normalization reuses the same pure decision core while review audit and video-state changes remain in their existing outer transaction.
