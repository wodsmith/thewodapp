...
Null `scalingLevelId` (open / no-division registration) is queried with `isNull(scoresTable.scalingLevelId)`, never an absent predicate — MySQL treats NULLs as distinct, so dropping the predicate leaks scores across divisions on a shared workout.
