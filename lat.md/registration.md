...
The athlete team roster page at [[apps/wodsmith-start/src/routes/compete/$slug/teams/$registrationId/index.tsx#TeamManagementPage]] treats `registrationId` as authoritative, redirects stale slugs to the registration's actual competition slug, and renders waiver status through [[apps/wodsmith-start/src/routes/compete/$slug/teams/$registrationId/-components/waiver-section.tsx#WaiverSection]] with the competition name in the signed-state copy. This keeps screenshots and old email links from making a qualifier waiver look like proof for a different competition.
