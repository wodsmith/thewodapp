...
7. Add-on (merch) selections validated when present — entitlement, availability, variant, quantity caps, soft stock; see [[commerce#Registration Add-ons]]
8. Fee calculated per division (division-specific fee overrides competition default)
9. Coupon applied if provided by link session data or manual entry
...
Registration windows do not require a close date. [[apps/wodsmith-start/src/utils/registration-window.ts#getRegistrationWindowStatus]] treats an opens-only window as open after the configured start day in the competition timezone, matching the server-side write guard that only blocks when a close date has actually passed.
...
The registration form validates teammate emails before checkout. [[apps/wodsmith-start/src/components/registration/use-registration-form.ts#buildTeammateEmailErrors]] rejects any teammate email that matches the signed-in captain's email, and [[apps/wodsmith-start/src/components/registration/registration-sections.tsx#TeamDetailsSection]] shows the field-level error while submit stays disabled. The server still enforces the same invariant inside [[apps/wodsmith-start/src/server/registration.ts#registerForCompetition]] for all entry points.
...
[[apps/wodsmith-start/src/server-fns/registration-fns.ts#cancelPendingPurchaseFn]] requires a purchase anchor, authenticated owner, and competition. It resolves the anchor to a checkout session, expires that hosted session, and cancels only its pending lines. Other sessions, accounts, competitions, and completed lines remain unchanged. An already expired session is safe to release on retry. A completed session or Stripe verification failure leaves reservations for settlement. Legacy cancel URLs without an anchor do not release reservations; normal expiry still applies. The registration loader treats cancellation as best-effort: Stripe failure or completed payment leaves the hold to settlement/expiry while the form loads normally.
...
Regression coverage is specified in [[checkout-safety-tests#Checkout Safety Tests]]. Timezone boundary cases retain the existing shared display rule and compare it with the actual submission date guards, including daylight-saving transitions in Los Angeles and Auckland.
