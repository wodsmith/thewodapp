...
Known competition context travels through every canonical command and writer target; a workout linked to multiple competitions cannot be resolved without it. The lock validates ownership through both event configuration and the workout's programming track, so manual scoring and sub-events do not require submission-window configuration. Both paths also participate in shared-score ownership checks. Unbound ambiguous lookups reject instead of choosing a first row.
