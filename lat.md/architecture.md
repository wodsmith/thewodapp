...
The public schedule page never receives unpublished heats: [[apps/wodsmith-start/src/server-fns/competition-heats-fns.ts#getHeatsForCompetitionFn]] filters out heats with a null `schedulePublishedAt` unless the viewer is a site admin or holds owner/admin/`manage_programming` on the organizing team, so draft heats — and the athlete names on them — never reach public clients. Its internal queries and those of `getPublicScheduleDataFn`/`getPublicEventHeatsFn` run in dependency-ordered parallel batches.
