...
[[apps/wodsmith-start/src/agents/organizer-file-import-agent.ts#OrganizerFileImportAgent]] reads a file dragged onto an organizer page and drafts import proposals for review — it classifies, extracts, validates, and proposes, but never writes.

When an organizer drops a roster/spreadsheet on a drop-enabled page, [[apps/wodsmith-start/src/components/organizer-import/import-shell.tsx#ImportShell]] (mounted around the competition layout's `Outlet`) creates a durable [[apps/wodsmith-start/src/db/schemas/agent-imports.ts#agentImportRunsTable]] row, uploads the file privately to R2 through the [[apps/wodsmith-start/src/routes/api/agent-import/upload.ts#Route]] endpoint (no public URL is ever emitted — volunteer PII stays server-side), then connects to the agent instance named `${importRunId}__${userId}` (the [[apps/wodsmith-start/src/server.ts]] WebSocket route validates the user-id suffix exactly like the judge agent).

The agent parses the file deterministically (CSV/TSV/text via `papaparse`; XLSX/PDF deferred) in [[apps/wodsmith-start/src/lib/organizer-file-import/parse.ts#parseImportFile]], loads existing volunteers/events for dedup matching, and runs a proposal-only `generateText` loop through AI Gateway with the same Workers AI model as the judge scheduler. Proposal tools run the pure validators in [[apps/wodsmith-start/src/lib/organizer-file-import/validate.ts#reconcileVolunteerProposal]] and stream volunteer/event proposals into state with inline match info (`already a volunteer`, matched on email) and warnings (`no email`). It supports refine-by-prompt (the `refine` callable re-runs with the current draft plus a natural-language instruction) and calls `ask_clarification` when the file's contents don't match the page. A re-dropped identical file (matched by checksum via [[apps/wodsmith-start/src/server/organizer-file-import/context.ts#findPriorAppliedImport]]) surfaces a duplicate-import warning so the organizer doesn't double-apply.

Writes happen only on an explicit organizer confirm, through [[apps/wodsmith-start/src/server-fns/organizer-file-import-fns.ts#applyOrganizerImportFn]], which re-checks access, creates volunteer invitations via the existing invite flow, skips duplicates, fails rows with no email, records what it wrote on the run row for idempotent re-apply, and returns per-row results for the receipt. [[apps/wodsmith-start/src/server-fns/organizer-file-import-fns.ts#undoImportFn]] reverses the created (still-pending) invitations. Access + entitlement are enforced in [[apps/wodsmith-start/src/server/organizer-file-import/access.ts#requireFileImportTeamAccess]] (request context) and [[apps/wodsmith-start/src/server/organizer-file-import/access.ts#requireFileImportAgentAccess]] (Durable Object), gated by `ai_file_import` ([[apps/wodsmith-start/src/config/features.ts#FEATURES]] → `AI_FILE_IMPORT`). The decision logic for both kinds is pure and unit-tested ([[apps/wodsmith-start/src/lib/organizer-file-import/validate.ts#planVolunteerApply]], [[apps/wodsmith-start/src/lib/organizer-file-import/validate.ts#planEventApply]]); the server fn only performs IO. The drop is enabled on the Volunteers and Judges pages (volunteer invites), the Events list page (event create — via [[apps/wodsmith-start/src/server-fns/competition-workouts-fns.ts#createWorkoutAndAddToCompetitionFn]], undone with [[apps/wodsmith-start/src/server-fns/competition-workouts-fns.ts#removeWorkoutFromCompetitionFn]]), and the event detail page (single-event update — via [[apps/wodsmith-start/src/server-fns/competition-workouts-fns.ts#saveCompetitionEventFn]], shown as an inline field diff and undone by restoring the recorded before-snapshot).
