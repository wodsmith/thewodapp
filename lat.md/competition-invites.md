...
The same seeder adds six Phase-2 `competition_invites` rows, one per lifecycle state, so the invite-row shapes are inspectable in Drizzle Studio without manual editing: a pending source-derived invite for Mike, an `accepted_paid` invite for Ryan (activeMarker still "active", `claimToken` nulled), an `expired` invite for Alex (activeMarker NULL), a `declined` invite for Sarah, a draft bespoke invite ("active but `claimToken = NULL`" shape), and a sent bespoke invite. Two rows keep a live `claimToken` for click-through testing: Mike's pending source invite (token `seed-invite-mike-pending-men-rx-phase2`) and the sponsored athlete's sent bespoke invite (token `seed-invite-sponsor-bespoke-women-rx-phase2`). The seeder logs both on run — the column stores the plaintext directly so a dev can copy it straight into a `/claim/<token>` URL without re-running the seed. The other four rows (Ryan, Alex, Sarah, draft-champion) all leave `claimToken` NULL, so they're inspectable as-is but not linkable.
