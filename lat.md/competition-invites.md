...
Claim tokens are 32-byte URL-safe random strings stored plaintext in `competition_invites.claimToken`, mirroring the `team_invitations.token` pattern. Lookups go through `eq(claimToken, ...)` directly — there is no hash. Helpers live in [[apps/wodsmith-start/src/lib/competition-invites/tokens.ts]].

The token is an unguessable identifier, not a bearer password. Email-locked claim (`identityMatch`) remains the actual auth gate; the token is what the athlete clicks (and what the organizer can copy from the invites table to share a link directly) but it can't be used to hijack a different account. Terminal transitions null the token on the row so a stale link can't replay.

[[apps/wodsmith-start/src/lib/competition-invites/tokens.ts#generateInviteClaimTokenPlaintext]] uses `crypto.getRandomValues` + base64url-no-padding (via `@oslojs/encoding`) so every character carries entropy and the result is safe in a URL path. Pure function — safe to call from server functions, workflows, queue consumers, and tests.
...
[[apps/wodsmith-start/src/server-fns/competition-invite-fns.ts#listActiveInvitesFn]] returns a minimal `ActiveInviteSummary` projection rather than full table rows — the organizer client only needs identity + classification info, so we never serialize the raw `claimToken` to the browser by default. A `hasClaimToken` boolean carries forward the "draft vs already-sent" distinction the bespoke section needs; the full plaintext token is fetched on demand via a separate server fn when the organizer asks for a copy-link.
...
The send pipeline is split into three transitions so callers can compose them around their own work: [[apps/wodsmith-start/src/server/competition-invites/rounds.ts#beginSendingRound]] flips `{draft, failed} → sending` (the double-click defense; `failed` is accepted so an organizer can retry from a crashed send without manual DB edits), the caller runs its insert/enqueue logic, and finalization is either [[apps/wodsmith-start/src/server/competition-invites/rounds.ts#finalizeRoundSend]] (`sending → sent`, sets `recipientCount` + `sentAt` + `sentByUserId`) or [[apps/wodsmith-start/src/server/competition-invites/rounds.ts#markRoundFailed]] (`sending → failed`, recoverable retry). We don't move backwards to `draft` from `failed` because partial invite rows may already exist; the next `beginSendingRound` reuses the same row so retries don't fragment the timeline.

The send orchestrator [[apps/wodsmith-start/src/server-fns/competition-invite-fns.ts#issueInvitesFn]] wraps every step between `beginSendingRound` and `finalizeRoundSend` (lookups, `issueInvitesForRecipients`, draft re-issue loop, render + queue.send) in a single failure guard that calls `markRoundFailed` on any throw — otherwise a crash anywhere in that band would leak `sending` state and the conflict guard would block all future sends.

[[apps/wodsmith-start/src/server/competition-invites/rounds.ts#revokeActiveInvitesForEmails]] is the pre-send revoke used by R2-supersedes-R1 flows: it transitions every still-pending active invite for a given (championship, division, email-set) to `revoked` (nulls `claimToken` + `activeMarker`) inside the caller's transaction, so the new round's inserts don't collide with the unique-active index. `accepted_paid` rows are left alone — re-inviting a paid athlete is a no-op anyway.
...
[[apps/wodsmith-start/src/server-fns/competition-invite-fns.ts#revokeInviteFn]] flips a single pending invite to `revoked`, nulls `claimToken` + `activeMarker`, and rejects when the invite is no longer pending. Used by the roster row action.
