...
The same seeder adds six Phase-2 `competition_invites` rows, one per lifecycle state, so the invite-row shapes are inspectable in Drizzle Studio without manual editing: a pending source-derived invite for Mike, an `accepted_paid` invite for Ryan (activeMarker still "active", claim token nulled), an `expired` invite for Alex (activeMarker NULL), a `declined` invite for Sarah, a draft bespoke invite ("active but no token" shape), and a sent bespoke invite. Two rows keep a live `claim_token_hash` for click-through testing: Mike's pending source invite (token plaintext `seed-invite-mike-pending-men-rx-phase2`) and the sponsored athlete's sent bespoke invite (token plaintext `seed-invite-sponsor-bespoke-women-rx-phase2`). The seeder logs both on run — SHA-256 of those strings reproduces the `claim_token_hash` column so a dev can recover a working claim URL without re-running the seed. The other four rows (Ryan, Alex, Sarah, draft-champion) all null the hash on entry, so they're inspectable as-is but not linkable.
