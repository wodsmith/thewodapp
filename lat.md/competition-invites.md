...
The organizer route is [[apps/wodsmith-start/src/routes/compete/organizer/$competitionId/invites/index.tsx]]. It renders six tabs — Candidates, Sources, Sent, Round History, Email Templates, Series Global — with Candidates, Sources, and Sent live and the others placeholders for later phases.
...
4. For each `alreadyActive` row flagged `isDraft: true` calls [[apps/wodsmith-start/src/server/competition-invites/issue.ts#reissueInvite]] to activate / re-deliver — rotates in a fresh token, sets `expiresAt`, bumps `sendAttempt`.
