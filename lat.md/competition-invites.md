...
[[apps/wodsmith-start/src/server/competition-invites/issue.ts#issueInvitesForRecipients]] runs in a single transaction: it snapshots existing active invites for the recipient emails, runs each through [[apps/wodsmith-start/src/server/competition-invites/issue.ts#classifyExistingInvite]] (the pure decision helper), and either reissues / skips / supersedes-and-inserts based on the result. Each new row carries a freshly generated token hash, `activeMarker = "active"`, `status = "pending"`, and `sendAttempt = 0`. The return shape `{ inserted, alreadyActive, supersededInviteIds }` hands the caller the *plaintext* tokens for the just-inserted rows — the only moment the plaintext exists in process memory before it's templated into an outgoing email — and the ids of any prior invites the same transaction revoked.

`supersedeActivePendingInvites` (default true) is the R2-supersedes-R1 atomic-revoke flag from ADR-0011 Phase 3. When an athlete with an active *pending+token* R1 invite is included in R2's recipients, the prior row is transitioned to `revoked` (nulls `claimTokenHash` + `activeMarker`, sets `revokedByUserId` from `superseededByUserId`) inside the same transaction as the R2 insert. So each athlete keeps at most one active invite per (championship, division), regardless of origin. `accepted_paid` rows are never superseded (the athlete is registered); drafts are never superseded (they reissue in place); explicit-revoked / declined / expired rows have `activeMarker = NULL` and don't enter the classifier at all.
