...
- Depends on: Plan 001 track experience, already represented in the inspected main revision, and ownership PR #691 (`zac/training-access-boundaries`, inspected head `95afbceb1c6c19feddbd8405548bc100363d5584`) or its merged equivalent.
...
Ownership follow-up: PR #691 is **not merge-ready** while its owner addresses review cases. The owner reports forthcoming `canReadWorkout(id)` protection for indirect source metadata/date lookups and batched requested-team membership filtering. These additions were reported, not independently inspected in this reconciliation. Before implementation, obtain the final reviewed head from coordinator `01a07dff-e59b-72c1-8e71-24d8468e8dd6`, inspect those helpers and tests, and update the drift baseline. Reuse `canReadWorkout` for indirect reads where appropriate; do not freeze implementation against the earlier inspected head. Personal score writes remain this plan's responsibility, separate from PR #691.
