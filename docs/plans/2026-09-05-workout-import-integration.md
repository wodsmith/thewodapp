...
| Runtime compatibility | Main/Crew scheduler tests, types and Worker builds with pinned stack | Main client/Worker build passes with final pinned tooling and an 8 GB Node heap; 19 runtime tests, main 144 tests and Crew type check/22 scheduler tests pass; normal pre-push all-package type checks pass |
