...
| `claimTokenHash` | varchar(64) | SHA-256 of the URL-safe token. The plaintext token only appears in the outgoing email. |
| `claimTokenLast4` | varchar(8) | For organizer support so they can confirm an athlete clicked the right link. |
| `expiresAt` | datetime | Mirrors `round.rsvpDeadlineAt` at send time, but stored per-invite so per-invite extensions work. |
| `status` | varchar(20) | `"pending"` \| `"accepted"` \| `"accepted_paid"` \| `"declined"` \| `"expired"` \| `"revoked"` — first five mirror the mockup's `STATUS_META` keys in `project/invites/leaderboard.jsx`. |
