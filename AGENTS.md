...
This project is indexed by GitNexus as **thewodapp** (30925 symbols, 69010 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- **MUST run `detect_changes({scope: "all"})` before committing** to verify your changes only affect expected symbols and execution flows. For regression review, compare against the default branch: `detect_changes({scope: "compare", base_ref: "main"})`.
...
- NEVER commit changes without running `detect_changes({scope: "all"})` to check affected scope.
