...
async function main() {
  ...
  try {
    ...
    const pnpmInstall = await sprite.execFile("bash", ["-c", "curl -fsSL https://get.pnpm.io/install.sh | bash"]);
  ...
  try {
    ...
    const bunInstall = await sprite.execFile("bash", ["-c", "curl -fsSL https://bun.sh/install | bash"]);
  ...
  try {
    ...
    await sprite.execFile("bash", ["-c", "which claude && claude install || echo 'claude not found, skipping'"]);
  } catch {
