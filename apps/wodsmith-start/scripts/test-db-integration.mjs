...
import { mysqlTestConfig } from "../test/integration/mysql-test-config.ts"
...
try {
  ...
  url.username = String(mysqlTestConfig.user ?? "root")
  url.password = String(mysqlTestConfig.password ?? "")
  if (mysqlTestConfig.socketPath) url.searchParams.set("socketPath", mysqlTestConfig.socketPath)
  else {url.hostname = mysqlTestConfig.host; url.port = String(mysqlTestConfig.port)}
  ...
  exitCode = await new Promise((resolve,reject)=>{
    const child = spawn("pnpm", args, {stdio:"inherit",env:{...process.env,WODSMITH_TEST_MYSQL_REQUIRED:"1",TRAINING_TEST_DATABASE_URL:url.toString()}})
