...
try {
  ...
  await expect(page.getByRole("button", { name: "Workout import access required", exact: true })).toBeDisabled()
  ...
  await page.reload()
  await page.waitForLoadState("networkidle")
  await expect(page.getByRole("button", { name: "Import workout", exact: true })).toBeEnabled()
