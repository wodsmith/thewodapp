...
export const WORKOUT_IMPORT_IMAGE_TYPES = [
  "image/png",
  "image/jpeg",
  "image/webp",
] as const
...
export type WorkoutImportDestination = z.infer<
  typeof workoutImportDestinationSchema
>

/** Browser/model data has no ownership, visibility, or persistence IDs. */
export const workoutImportWorkoutSchema = z
  .object({
...
export const workoutImportQuestionSchema = z
  .object({
    id,
    field: z.enum([
      "name",
...
export const workoutImportProposalSchema = z
  .object({
...
export const workoutImportSourceSchema = z
  .object({
...
export const workoutImportRevisionInputSchema = z
  .object({
    importId: id,
    requestId: id,
  ...
  .strict()
export type WorkoutImportRevisionInput = z.infer<
  typeof workoutImportRevisionInputSchema
>
...
export const normalizedWorkoutSaveSchema = workoutImportWorkoutSchema
  .extend({
  ...
  .superRefine((v, ctx) => {
    if (v.scheme === "time-with-cap" && v.timeCapSeconds === null)
      ctx.addIssue({
        code: "custom",
        path: ["timeCapSeconds"],
        message: "Capped time requires a cap in seconds",
      })
    if (v.scheme !== "time-with-cap" && v.timeCapSeconds !== null)
      ctx.addIssue({
        code: "custom",
        path: ["timeCapSeconds"],
        message:
          "Duration belongs in the prescription; only capped time has a time cap",
      })
    if (v.roundsToScore > 1 && v.scoreType === null)
      ctx.addIssue({
        code: "custom",
        path: ["scoreType"],
        message: "Choose how separately recorded scores are combined",
      })
...
export const workoutImportSaveInputSchema = z
  .object({
    importId: id,
    revision: z.number().int().min(1),
    idempotencyKey: id,
    ...
    track: z
      .object({
        trackOrder: z.number().int().min(1).max(9999),
  ...
  .strict()
export type WorkoutImportSaveInput = z.infer<
  typeof workoutImportSaveInputSchema
>
export interface WorkoutImportSaveResult {
  workoutId: string
