...
export function PricingSettingsForm({
	...
	const updateCompetitionFeeConfigMutation = useServerFnMutation(
		updateCompetitionFeeConfigFn,
	)
	const updateDivisionFeeMutation = useServerFnMutation(updateDivisionFeeFn)

	// Handle default fee save
	const handleSaveDefaultFee = async () => {
		setIsSubmitting(true)
		try {
			await updateCompetitionFeeConfigMutation.mutateAsync({
	...
	const handleDivisionFeeUpdate = async (
		...
		setIsSubmitting(true)
		try {
			await updateDivisionFeeMutation.mutateAsync({
				competitionId: competition.id,
				divisionId,
