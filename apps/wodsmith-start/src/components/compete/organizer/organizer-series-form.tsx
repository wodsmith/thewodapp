...
import { createCompetitionGroupFn } from "~/server-functions/competitions"
...
export function OrganizerSeriesForm({
	...
	const createGroupMutation = useServerFnMutation(createCompetitionGroupFn, {
		onError: (error) => {
			const message =
			...
			<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
				...
				<div className="flex gap-4">
					<Button type="submit" disabled={createGroupMutation.isPending}>
						{createGroupMutation.isPending ? "Creating..." : "Create Series"}
					</Button>
					<Button
						type="button"
						variant="outline"
