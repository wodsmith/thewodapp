...
export const deletePasskeyFn = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		// Verify the passkey belongs to the current user before deleting
		const passkeyToDelete = passkeys.find(
			(p) => p.credentialId === data.credentialId,
		)
		if (!passkeyToDelete) {
			throw new Error("Passkey not found")
		}

		// Delete the passkey (scoped to user for security)
		await db
			.delete(passKeyCredentialTable)
			.where(
				and(
					eq(passKeyCredentialTable.credentialId, data.credentialId),
					eq(passKeyCredentialTable.userId, session.user.id),
				),
			)
