...
import {chunk, SQL_BATCH_SIZE, autochunk} from '@/utils/batch-query'
import {
  decodeScore,
  encodeScore,
  encodeRounds,
 ...
 */
export const getEventScoreEntryDataWithHeatsFn = createServerFn({method: 'GET'})
  .inputValidator((data: unknown) =>
    getEventScoreEntryDataInputSchema.parse(data),
  )
...
export const saveCompetitionScoresFn = createServerFn({method: 'POST'})
  ...
  .handler(
    async ({
      ...
      success: boolean
      data: {savedCount: number; errors: Array<{userId: string; error: string}>}
    }> => {
