...
import {
	assignJudgeToHeatFn,
	bulkAssignJudgesToHeatFn,
	moveJudgeAssignmentFn,
	removeJudgeFromHeatFn,
} from "@/server-fns/judge-scheduling-fns"
...
export function JudgeHeatCard({
	...
	async function handleAssign() {
		...
		setIsAssigning(true)
		try {
			const result = await assignJudgeToHeatFn({
				data: {
					heatId: heat.id,
					competitionId,
					organizingTeamId,
	...
	async function handleDropAssign(membershipIds: string[], startLane: number) {
		...
		if (assignments.length === 1) {
			...
			try {
				const result = await bulkAssignJudgesToHeatFn({
					data: {
						heatId: heat.id,
						competitionId,
						organizingTeamId,
							...
							<div className="space-y-4">
								...
								<div className="flex justify-end gap-2">
									...
									<Button
										...
										disabled={
											!selectedMembershipId || !selectedLane || isAssigning
										}
									>
										{isAssigning && (
