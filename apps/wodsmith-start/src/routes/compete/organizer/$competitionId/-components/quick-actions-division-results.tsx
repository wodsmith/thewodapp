        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <CardDescription>
                {autoPublish
                  ? "Results publish automatically as scores come in"
                  : `${divisionResults.totalPublishedCount} of ${divisionResults.totalCombinations} published`}
              </CardDescription>
            </div>
          </div>
          {supportsAutoPublish && (
            <Select
              value={autoPublish ? "auto" : "manual"}
              onValueChange={(value) => handleSetAutoPublish(value === "auto")}
              disabled={isAutoPublishPending}
            >
              <SelectTrigger className="w-full sm:w-[180px] h-8 text-xs">
                <SelectValue>
                  {isAutoPublishPending ? (
        ...
        <CardContent>
          ...
          {divisionResults.events.length > 0 && (
            <div className="mt-4 pt-4 border-t">
              <p className="text-xs text-muted-foreground">
                Published divisions show results on the public leaderboard.
                Draft divisions are hidden from athletes. Results are published
                per event, allowing you to release results as each event
                completes.
              </p>
            </div>
          )}
