...
import {getDivisionResultsStatusFn} from '@/server-fns/division-results-fns'
...
export const Route = createFileRoute('/compete/organizer/$competitionId/')({
  ...
  loader: async ({params}) => {
    ...
    return {
      ...
      divisionResults: divisionResultsResult,
      organizingTeamId: competition.organizingTeamId,
    }
  },
})
...
function CompetitionOverviewPage() {
  ...
  // Calculate overall publishing status
  const publishedEventsCount = events.filter(
    (e) => e.eventStatus === 'published',
  ).length
  const publishedHeatsCount = events.filter(
    (e) => e.heatStatus === 'published',
  ).length
  const eventsWithHeats = events.filter((event) =>
    heats.some((h) => h.trackWorkoutId === event.id && h.scheduledTime),
  ).length

  // Calculate publishing completion percentage
  const totalItems =
    events.length + eventsWithHeats + divisionResults.totalCount
  const publishedItems =
    publishedEventsCount + publishedHeatsCount + divisionResults.publishedCount
  const publishingProgress =
    totalItems > 0 ? Math.round((publishedItems / totalItems) * 100) : 0

  return (
    <>
      {/* Quick Actions - Publishing Section */}
      {events.length > 0 && (
            ...
            {totalItems > 0 && (
              <CardContent>
                ...
                <div className="mt-2 flex justify-between text-xs text-muted-foreground">
                  <span>
                    Events: {publishedEventsCount}/{events.length}
                  </span>
                  {eventsWithHeats > 0 && (
                    <span>
                      Heats: {publishedHeatsCount}/{eventsWithHeats}
                    </span>
                  )}
                  {divisionResults.totalCount > 0 && (
                    <span>
                      Results: {divisionResults.publishedCount}/
                      {divisionResults.totalCount}
                    </span>
                  )}
                </div>
              </CardContent>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            ...
            {divisionResults.totalCount > 0 && (
              <QuickActionsDivisionResults
                competitionId={competition.id}
                organizingTeamId={organizingTeamId}
                divisions={divisionResults.divisions}
                publishedCount={divisionResults.publishedCount}
                totalCount={divisionResults.totalCount}
              />
            )}
          </div>
        </div>
