/**
 * Competition Athletes Page
 *
 * Shared page body for the organizer and cohost athletes routes. The organizer
 * route renders it with defaults (organizer server fns, registration detail
 * links, refund actions); the cohost route injects cohost-permissioned
 * mutation callbacks and gates edit UI behind cohost permissions.
 *
 * Cohost gating (driven by the optional `permissions` prop — undefined means
 * full access):
 * - "Add registration" button, per-row actions menu, the actions table column,
 *   and the "Form Questions" tab are hidden without `editRegistrations`.
 * - A `tab=registration-rules` search param is coerced to `athletes` when
 *   `editRegistrations` is false so <Tabs> never holds a value with no
 *   matching TabsContent.
 * - Registration detail links render only when `registrationDetailRoute` is
...
import {
  ...
  type getPendingTransfersForCompetitionFn,
} from "@/server-fns/purchase-transfer-fns"
import {
  refundRegistrationFn,
  removeRegistrationFn,
} from "@/server-fns/registration-fns"
import type { getCompetitionRegistrationAnswersFn } from "@/server-fns/registration-questions-fns"
import type { getCompetitionWaiversFn } from "@/server-fns/waiver-fns"
...
export const athletesSortColumns = [
  "name",
  "division",
  "teamName",
  "affiliate",
...
export type AthletesSortColumn = (typeof athletesSortColumns)[number]
export type AthletesSortDirection = "asc" | "desc"

export const athletesSearchSchema = z.object({
  tab: z
  ...
  // Sorting
  sortBy: z.enum(athletesSortColumns).optional(),
  sortDir: z.enum(["asc", "desc"]).optional(),
})

type OrganizerRegistrationsResult = Awaited<
  ReturnType<typeof getOrganizerRegistrationsFn>
>
type ManualRegistrationDialogProps = ComponentProps<
  typeof ManualRegistrationDialog
>
type TransferDivisionDialogProps = ComponentProps<typeof TransferDivisionDialog>
type TransferRegistrationDialogProps = ComponentProps<
  typeof TransferRegistrationDialog
>
...
interface AthletesPageProps {
  ...
  teamId: string
  tab: "athletes" | "registration-rules"
  registrations: OrganizerRegistrationsResult["registrations"]
  /** Organizer-only; cohost loaders don't return refund data, hiding refund UI. */
  canRefund?: OrganizerRegistrationsResult["canRefund"]
  /** Organizer-only; cohost loaders don't return refund data, hiding refund badges. */
  refundsByPurchaseId?: OrganizerRegistrationsResult["refundsByPurchaseId"]
  divisions: ManualRegistrationDialogProps["divisions"]
  questions: ManualRegistrationDialogProps["questions"]
  answersByRegistration: Awaited<
    ReturnType<typeof getCompetitionRegistrationAnswersFn>
  >["answersByRegistration"]
  waivers: Awaited<ReturnType<typeof getCompetitionWaiversFn>>["waivers"]
  signaturesByUser: Record<string, Date>
  pendingInvites: PendingTeammateInvite[]
  pendingTransfers: Awaited<
    ReturnType<typeof getPendingTransfersForCompetitionFn>
  >
  currentDivisionFilter: string | undefined
  currentQuestionFilters: Record<string, string[]>
  currentWaiverFilters: string[]
  currentSortBy: AthletesSortColumn | undefined
  currentSortDir: AthletesSortDirection | undefined
  /** Cohost permission metadata; undefined = full access (organizer). */
  permissions?: CohostMembershipMetadata
  /**
   * Route for registration row "View details" links. Organizer routes pass
   * their detail route; cohost routes omit it (no cohost equivalent exists),
   * which hides the links.
   */
  registrationDetailRoute?: "/compete/organizer/$competitionId/athletes/$registrationId"
  /** Cohost routes inject a cohost-permissioned manual registration fn. */
  onCreateRegistration?: ManualRegistrationDialogProps["onCreateRegistration"]
  /** Cohost routes inject a cohost-permissioned division transfer fn. */
  onTransferDivision?: TransferDivisionDialogProps["onTransferDivision"]
  /** Cohost routes inject a cohost-permissioned removal fn. */
  onRemoveRegistration?: (data: {
    registrationId: string
    competitionId: string
  }) => Promise<unknown>
  /** Cohost routes disable registration transfers (no cohost transfer fn). */
  transferRegistrationDisabled?: TransferRegistrationDialogProps["disabled"]
  transferRegistrationDisabledMessage?: TransferRegistrationDialogProps["disabledMessage"]
  /** Cohost routes inject cohost-permissioned question CRUD/reorder fns. */
  questionOverrides?: ComponentProps<
    typeof RegistrationQuestionsEditor
  >["overrides"]
}
...
export function AthletesPage({
  ...
  transferRegistrationDisabledMessage,
  questionOverrides,
}: AthletesPageProps) {
  // Cohost permission gating — undefined permissions = full access (organizer).
  const canEditRegistrations = permissions
    ? permissions.editRegistrations === true
    : true
  /**
  ...
  const removeRegistrationDefault = useServerFn(removeRegistrationFn)
  ...
  const handleRemoveRegistration = async () => {
    ...
    setIsRemoving(true)
    try {
      if (onRemoveRegistration) {
        await onRemoveRegistration({
          registrationId: removingRegistration.id,
          competitionId: competition.id,
        })
      } else {
        await removeRegistrationDefault({
          data: {
            registrationId: removingRegistration.id,
            competitionId: competition.id,
          },
  ...
  const handleSort = (column: AthletesSortColumn) => {
    navigate({
      to: ".",
      search: (prev) => {
  ...
  const SortIcon = ({ column }: { column: AthletesSortColumn }) => {
    ...
    <>
            ...
            {questions.some((q) => q.source === "series") && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    These questions are inherited from the series and apply to
                    all competitions.{" "}
                    {seriesGroupId && (
                      <Link
                        to="/compete/organizer/series/$groupId"
                        params={{ groupId: seriesGroupId }}
      ...
      {transferTarget && (
        <TransferDivisionDialog
          ...
          registeredDivisionIds={registrations
            ...
            .map((r) => r.divisionId!)}
          onTransferDivision={onTransferDivision}
        />
      )}
      ...
      {transferRegistrationTarget && (
        <TransferRegistrationDialog
          ...
          competitionId={competition.id}
          disabled={transferRegistrationDisabled}
          disabledMessage={transferRegistrationDisabledMessage}
        />
      )}
    </>
  )
