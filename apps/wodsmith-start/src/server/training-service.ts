...
export function createTrainingService(
  ...
  const getDb = () => db
  type TrainingDb = TrainingDatabase
  ...
  async function requireTrainingAccess(
    ...
    assertTrainingTeam(actor, teamId)
  ...
  async function getTrainingContext(): Promise<TrainingContext> {
    ...
    const activeTeamId = actor.activeTeamId ?? null
    return {
      userId,
  ...
  async function getTrainingWeek(input: {
    teamId: string
    trackId: string
    startDate: string
    mode: "athlete" | "coach"
    ...
    assertTrainingScope(
      actor,
      input.mode === "coach" ? "programming:read" : "training:read",
    )
    ...
    const sessions = await getDb()
      ...
      .where(
        and(
          ...
          lte(
            trainingSessionsTable.trainingDate,
    ...
    try {
      return await db.transaction(async (tx) => {
        ...
        for (const block of content.blocks) {
          ...
          )?.blocks.find((stored) => stored.id === block.id)?.workout
        ...
        if (
          ...
          throw new Error(
            "The timezone of a published session cannot be changed",
          )
        await tx
  ...
  async function publishTrainingSession(input: {
    sessionId: string
  ...
  async function copyTrainingSession(input: {
    sessionId: string
    targetDate: string
  ...
  async function setTrainingCheer(input: {
    resultId: string
    cheered: boolean
  ...
  async function getTrainingHistory(input: {
    teamId: string
    trackId?: string
  }): Promise<OwnTrainingResult[]> {
    assertTrainingScope(actor, "training:read")
  ...
  async function getTrainingWorkoutOptions(input: { teamId: string }) {
    assertTrainingScope(actor, "programming:read")
    await requireTrainingAccess(input.teamId, undefined, true)
    return readTrainingWorkoutOptions(input.teamId)
  }

  async function getPersonalTrainingWorkoutOptions(input: { teamId: string }) {
    assertTrainingScope(actor, "training:read")
