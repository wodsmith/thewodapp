...
function createPersonalTrainingOperations(
  ...
  const getDb = () => db
  const { getTrainingContext, requireTrainingAccess } =
    createTrainingService(dependencies)
  const { requireTrackRead, workoutVisibilityCondition } =
    createTrainingReadAccess(db, actor.userId, actor.allowedTeamIds)
  type Db = TrainingDatabase
  ...
  async function saveTrainingPreference(input: {
    teamId: string
    defaultTrackId: string
  }): Promise<void> {
    assertTrainingScope(actor, "training:write")
  ...
  async function getPersonalTrainingDay(input: {
    teamId: string
    trainingDate: string
    trackId?: string
  }): Promise<PersonalTrainingDay> {
    assertTrainingScope(actor, "training:read")
  ...
  async function listTrainingLibraryWorkouts(input: {
    teamId: string
    search?: string
  }) {
    assertTrainingScope(actor, "training:read")
  ...
  async function getTrainingLibraryWorkout(input: {
    teamId: string
    workoutId: string
  ...
  > {
    ...
    if (data.sourceTrackId && !provenance) {
      ...
      if (!association || data.sourceDate)
        throw new Error(
          "FORBIDDEN: Published source occurrence is not available",
        )
    }
  ...
  async function savePersonalTrainingSession(
    input: SavePersonalTrainingSessionInput,
        ...
        if (data.mode === "append") {
          if (
            data.items.some((item) =>
              previous.some(
                (old) => old.id === item.id && !sameInput(old, item),
              ),
            )
          )
            throw new Error(
  ...
  async function lockOwnedItem(
    ...
    if (!item)
      throw new Error("NOT_FOUND: Workout is no longer in your session")
    return { session, item }
  }
  async function savePersonalTrainingResult(
  ...
  async function linkPersonalTrainingScore(input: {
    personalSessionId: string
    itemId: string
  ...
  async function savePersonalLibraryResult(input: {
    personalSessionId: string
    itemId: string
    expectedRevision: number
    score: string
  ...
  async function saveDirectLibraryResult(
  ...
  async function getDirectLibraryEntry(input: {
    teamId: string
    workoutId: string
    sourceTrackId?: string
    sourceDate?: string
  ...
  async function getPersonalTrainingHistory(input: {
    teamId: string
  }): Promise<OwnTrainingResult[]> {
    assertTrainingScope(actor, "training:read")
  ...
  async function getPersonalLibraryScalingLevels(input: {
    personalSessionId: string
    itemId: string
  }) {
    assertTrainingScope(actor, "training:read")
