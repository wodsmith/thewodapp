...
export async function copyTrainingSessionFn({data}: {data: {sessionId: string; targetDate: string; targetTrackId: string; expectedRevision: number}}) { const s = state.sessions.find(s => s.id === data.sessionId)!; if (s.revision !== data.expectedRevision) throw new Error("Reload the source session before copying."); if (state.sessions.some(t => t.teamId === s.teamId && t.trackId === data.targetTrackId && t.trainingDate === data.targetDate)) throw new Error("There is already a session on this day. Choose an empty day."); const draft = structuredClone(s.draft ?? s.published)!; draft.blocks.forEach(b => { b.id = crypto.randomUUID() }); const copy = {...s, id: crypto.randomUUID(), trackId: data.targetTrackId, trainingDate: data.targetDate, draft, published: null, publishedVersion: 0, revision: 1}; state.sessions.push(copy); persist(); return structuredClone(copy) }

export async function getTrainingWorkoutOptionsFn() {return {movements:[{id:"thruster",name:"Thruster",type:"weightlifting" as const},{id:"pullup",name:"Pull-up",type:"gymnastic" as const},{id:"row",name:"Row",type:"monostructural" as const}],scalingGroups:[]}}
