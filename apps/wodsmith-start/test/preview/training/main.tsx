...
const compete = createRoute({getParentRoute: () => rootRoute, path:"/compete-reference", component: () => {const [open,setOpen] = useState(true); return <main className="mx-auto max-w-6xl p-6"><h1 className="text-2xl font-bold">Competition events</h1><button type="button" onClick={() => setOpen(true)}>Create event</button><CreateEventDialog open={open} onOpenChange={setOpen} onCreateEvent={async () => setOpen(false)} movements={[]} /></main>}})
