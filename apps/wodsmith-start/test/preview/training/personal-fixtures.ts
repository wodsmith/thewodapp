...
export async function savePersonalTrainingResultFn({data}:{data:SavePersonalTrainingResultInput}):Promise<OwnTrainingResult> { const session=state.sessions.find(item=>item.id===data.personalSessionId);const item=session?.items.find(item=>item.id===data.itemId);if(!session||!item||item.kind==="library")throw new Error("Workout missing.");const result:OwnTrainingResult={id:`personal-${item.id}`,sessionId:session.id,blockId:item.id,publishedVersion:1,userId:context.userId,userName:"Zac Jones",trainingDate:session.trainingDate,trackId:"",block:item.block,scoreValue:Number(data.score)||null,displayScore:data.score,scaling:"custom",modification:"",audience:"private",unit:data.unit,completed:data.completed,cheerCount:0,hasCheered:false,notes:data.notes};state.results=state.results.filter(entry=>entry.id!==result.id).concat(result);persist();return result }
