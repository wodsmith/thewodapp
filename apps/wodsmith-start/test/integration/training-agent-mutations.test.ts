...
const resultReply = z.object({
  ...
  saved: z.object({
    ...
    score: z.object({ id: z.string() }).nullable(),
  }),
})
...
describe.skipIf(!databaseUrl)("agent mutations on MySQL", () => {
  ...
  beforeAll(async () => {
    ...
    if (
      ...
      throw new Error("Disposable local training database required")
    pool = mysql.createPool(databaseUrl)
    db = createWodsmithDb(pool)
    state.db = db
    await db.insert(userTable).values([
      { id: userId, firstName: "Agent athlete" },
      { id: "agent_other", firstName: "Other" },
    ])
    await db
      .insert(teamTable)
      .values({
    ...
    await db
      ...
      .values({ id: "agent_scaling", title: "Agent scaling", isSystem: true })
    await db
  ...
  beforeEach(async () => {
    ...
    deps = {
      ...
      authorizeActor: async (executor) => {
        expect(executor).not.toBe(db)
        if (!live) throw new Error("FORBIDDEN: Grant revoked")
      },
    }
    ...
    await db
      ...
      .set({ isActive: true, roleId: "owner" })
      .where(eq(teamMembershipTable.id, "agent_owner_membership"))
    await db
  ...
  afterAll(async () => {
    await db
      ...
      .where(eq(scalingLevelsTable.id, "agent_rx"))
    await db
      .delete(scalingGroupsTable)
      .where(eq(scalingGroupsTable.id, "agent_scaling"))
  ...
  it("creates once across concurrent retries, detects web edits and archives owned definitions", async () => {
    ...
    await expect(
      call("create_workout", {
        ...input,
        workout: { ...workout, name: "Different" },
      }),
    ).rejects.toThrow("CONFLICT")
    const read = await call("get_workout", {
      teamId,
    ...
    await db
      .update(workouts)
      .set({ description: "Changed by web" })
    ...
    const current = await call("get_workout", {
      teamId,
      workoutId: created.workout.id,
    })
    ...
    expect(await call("delete_workout", remove)).toEqual(
      await call("delete_workout", remove),
    )
    expect((await db.select().from(workouts))[0].archivedAt).not.toBeNull()
    await expect(
      call("get_workout", { teamId, workoutId: created.workout.id }),
  ...
  it("hides archived definitions from web selection and retains an in-flight scheduler's reference", async () => {
    const created = workoutReply.parse(
      await call("create_workout", {
        teamId,
        idempotencyKey: "race-create",
        workout,
      }),
    )
    const selected = await getWorkoutsFn({
      data: { teamId, page: 1, pageSize: 20 },
    })
    expect(selected.workouts.map((item) => item.id)).toContain(
      created.workout.id,
    )
    // Deterministic legacy scheduler interleaving: read, archive, then insert.
    const [readBeforeArchive] = await db
      .select()
      .from(workouts)
      .where(eq(workouts.id, created.workout.id))
    ...
    await db
      ...
      .values({
        id: "archive-in-flight",
        teamId,
        workoutId: readBeforeArchive.id,
    ...
    const joined = await db
      ...
      .where(eq(scheduledWorkoutInstancesTable.id, "archive-in-flight"))
    expect(joined[0].workout.name).toBe(workout.name)
    expect(
      (await getWorkoutsFn({ data: { teamId, page: 1, pageSize: 20 } }))
        .workouts,
    ).toEqual([])
  ...
  it("revalidates live authority on receipt replay and preserves the first trusted client origin", async () => {
    ...
    expect(first).toMatchObject({
      receipt: {
        origin: { kind: "agent", clientId: "client-one", grantId: "grant-one" },
      },
    })
    expect(
      await call("create_workout", input, {
        ...deps,
        actor: { ...deps.actor, grantId: "grant-two", clientId: "client-two" },
      }),
    ).toEqual(first)
    live = false
    ...
    live = true
    await expect(
      call("create_workout", input, { ...deps, authorizeActor: undefined }),
    ).rejects.toThrow("Live grant validation")
    await expect(
      call("create_workout", input, {
        ...deps,
        actor: { ...deps.actor, allowedTeamIds: [] },
      }),
    ).rejects.toThrow("outside the training grant")
    await expect(
      call("create_workout", input, {
        ...deps,
        actor: { ...deps.actor, userId: "agent_other" },
      }),
    ).rejects.toThrow("personal library")
    await db
      .update(teamMembershipTable)
      .set({ isActive: false })
      .where(eq(teamMembershipTable.id, "agent_owner_membership"))
  ...
  it("rolls back the mutation if its durable receipt cannot be inserted", async () => {
    await pool
      .promise()
      .query(
        "CREATE TRIGGER agent_receipt_failure BEFORE INSERT ON training_mutation_receipts FOR EACH ROW SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'receipt failure'",
      )
    try {
      await expect(
        call("create_workout", { teamId, idempotencyKey: "rollback", workout }),
      ).rejects.toThrow("UNAVAILABLE")
    } finally {
      await pool.promise().query("DROP TRIGGER agent_receipt_failure")
    }
    expect(await db.select().from(workouts)).toHaveLength(0)
    expect(await db.select().from(trainingMutationReceiptsTable)).toHaveLength(
      0,
    )
  })
  ...
  it("reads, revises and deletes only owned source results with stale web edit protection", async () => {
    ...
    const created = resultReply.parse(
      await call("create_result", { teamId, idempotencyKey: "source", entry }),
    )
    const target = {
      kind: "source",
      sessionId: "agent_source",
    ...
    const foreign = await call(
      "get_result",
      { teamId, target },
      { ...deps, actor: { ...deps.actor, userId: "agent_other" } },
    )
    expect(foreign.saved).toBeNull()
    await db
      .update(trainingResultsTable)
      .set({ notes: "edited in web" })
      .where(eq(trainingResultsTable.id, created.saved.result.id))
    await expect(
      call("update_result", {
        teamId,
    ...
    const updated = resultReply.parse(
      await call("update_result", {
        teamId,
        idempotencyKey: "update-result",
        entry: { ...entry, value: { ...entry.value, score: "31" } },
        expectedVersion: refreshed.version,
      }),
    )
    await db
      .insert(trainingCheersTable)
      .values({ resultId: created.saved.result.id, userId: "agent_other" })
    await call("delete_result", {
      teamId,
      idempotencyKey: "delete-result",
      target,
      expectedVersion: updated.version,
    })
  ...
  it("preserves library rounds through edits and removes only the owned attempt on deletion", async () => {
    ...
    const result = resultReply.parse(
      await call("create_result", { teamId, idempotencyKey: "rounds", entry }),
    )
    expect(await db.select().from(scoreRoundsTable)).toHaveLength(3)
    const week = await call("get_training_week", {
      teamId,
      trackId: "agent_track",
    ...
    expect(week).toMatchObject({
      personalDays: [
        expect.objectContaining({
          trainingDate: "2026-09-05",
          state: "projection",
          personalSession: expect.objectContaining({
    ...
    await call("delete_workout", {
      teamId,
      idempotencyKey: "archive-performed",
      workoutId: created.workout.id,
      expectedVersion: created.version,
    })
    await expect(
      call("create_result", {
        teamId,
        idempotencyKey: "archived-new-attempt",
        entry: { ...entry, value: { ...entry.value, itemId: "new-attempt" } },
      }),
    ).rejects.toThrow("FORBIDDEN")
    const changed = resultReply.parse(
      await call("update_result", {
        teamId,
        idempotencyKey: "new-rounds",
  ...
  it("resolves an archived definition only through an owned historical score", async () => {
    const created = workoutReply.parse(
      await call("create_workout", {
        teamId,
        idempotencyKey: "history-workout",
        workout,
      }),
    )
    const entry = {
    ...
    await db
      ...
      .where(eq(personalTrainingResultsTable.id, result.saved.result.id))
    await call("delete_workout", {
      teamId,
    ...
    expect(await getOwnedLogWorkoutFn({ data: { scoreId } })).toMatchObject({
      workout: { id: created.workout.id, name: workout.name },
    })
    // Legacy callers can leave an owned score pointing at an inaccessible workout.
    await db
      .insert(workouts)
      .values({
        id: "forged-history-private",
        teamId: "unavailable-team",
        name: "Secret",
    ...
    await db
      .update(scoresTable)
      .set({ workoutId: "forged-history-private" })
      .where(eq(scoresTable.id, scoreId))
    await expect(getOwnedLogWorkoutFn({ data: { scoreId } })).rejects.toThrow(
      "Owned workout history not found",
    )
  ...
  it("preserves an archived workout in an existing day while excluding it from new selection", async () => {
    const created = workoutReply.parse(
      await call("create_workout", {
        teamId,
        idempotencyKey: "planned-library",
        workout,
      }),
    )
    const item = {
    ...
    const saved = await call("save_personal_training_day", {
      teamId,
      idempotencyKey: "planned-day",
    ...
    await call("delete_workout", {
      teamId,
      idempotencyKey: "archive-planned",
      workoutId: created.workout.id,
      expectedVersion: created.version,
    })
    const edited = await call("save_personal_training_day", {
      teamId,
      idempotencyKey: "edit-planned",
      trainingDate: "2026-09-07",
      expectedRevision: first.revision,
      items: [{ ...item, role: "strength" }],
    })
    expect(edited).toMatchObject({
      session: { items: [{ ...(first.items[0] as object), role: "strength" }] },
    })
  })
  ...
  it("separates personal composition, draft and publish scopes with replay-safe revisions", async () => {
    const personal = await call("save_personal_training_day", {
      teamId,
      idempotencyKey: "day",
    ...
    await expect(
      call("publish_programming", publish, {
        ...deps,
        actor: { ...deps.actor, scopes: ["programming:write"] },
      }),
    ).rejects.toThrow("programming:publish")
    ...
    await db
      ...
      .set({ roleId: "member" })
      .where(eq(teamMembershipTable.id, "agent_owner_membership"))
    await expect(call("publish_programming", publish)).rejects.toThrow(
      "FORBIDDEN",
    )
  })
  ...
  it("requires programming read scope and current programmer authority for a known draft receipt", async () => {
    const saved = await call("save_programming_draft", {
      teamId,
      idempotencyKey: "private-draft",
      trackId: "agent_track",
      trainingDate: "2026-09-06",
      timezone: "UTC",
      expectedRevision: 0,
    ...
    const receiptId = z
      .object({ receipt: z.object({ id: z.string() }) })
      .parse(saved).receipt.id
    const read = { teamId, receiptId }
    expect(await call("get_mutation_receipt", read)).toMatchObject({
      result: saved,
    })
    await expect(
      call("get_mutation_receipt", read, {
        ...deps,
        actor: { ...deps.actor, scopes: ["training:read"] },
      }),
    ).rejects.toThrow("programming:read")
    await db
      .update(teamMembershipTable)
      .set({ roleId: "member" })
      .where(eq(teamMembershipTable.id, "agent_owner_membership"))
    await expect(call("get_mutation_receipt", read)).rejects.toThrow(
      "FORBIDDEN",
    )
  })
  ...
  it("advertises only permitted executable operations with serializable canonical schemas", () => {
    const reads = listAgentOperations({
      ...deps.actor,
      scopes: ["training:read"],
    })
    expect(reads.every((operation) => operation.annotations.readOnlyHint)).toBe(
      true,
    )
    expect(reads.map((operation) => operation.name)).not.toContain(
      "create_workout",
    )
    expect(() =>
      listAgentOperations({ userId, scopes: ["training:read"] }),
    ).toThrow("Incomplete")
    expect(
      listAgentOperations(deps.actor).find(
        (operation) => operation.name === "update_workout",
      )?.annotations.destructiveHint,
    ).toBe(true)
    for (const operation of listAgentOperations(deps.actor))
