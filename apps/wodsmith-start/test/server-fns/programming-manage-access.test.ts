...
beforeEach(() => {
  const chain = { from: () => chain, leftJoin: () => chain, where: () => chain, limit: async () => [{ id: "track", ownerTeamId: "owner-team", isPublic: 1 }] }
...
it("uses site administration for the managed CrossFit source track", async () => {
  const chain = { from: () => chain, leftJoin: () => chain, where: () => chain, limit: async () => [{ id: CROSSFIT_TRACK_ID, ownerTeamId: "crossfit-team", isPublic: 1 }] }
  mock.select.mockReturnValue(chain)
