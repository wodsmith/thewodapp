...
import {
  assignWorkoutSponsorFn,
  createSponsorFn,
  createSponsorGroupFn,
  deleteSponsorFn,
  deleteSponsorGroupFn,
  getCompetitionSponsorGroupsFn,
...
import type {Competition, TrackWorkout} from '@/db/schema'
...
describe('Sponsor Server Functions (TanStack)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
    mockDb.reset()
  })

  describe('getSponsorFn', () => {
    it('returns sponsor by ID', async () => {
      ...
      const result = await getSponsorFn({data: {sponsorId: 'sponsor-1'}})

      expect(result.sponsor).toEqual(sponsor)
    ...
    it('returns null when sponsor not found', async () => {
      ...
      const result = await getSponsorFn({data: {sponsorId: 'nonexistent'}})

      expect(result.sponsor).toBeNull()
    })
  })

  describe('getCompetitionSponsorsFn', () => {
    it('returns organized sponsors with groups and ungrouped', async () => {
      ...
      const result = await getCompetitionSponsorsFn({
        data: {competitionId},
      })
  ...
  describe('getCompetitionSponsorGroupsFn', () => {
    it('returns ordered list of sponsor groups', async () => {
      ...
      const result = await getCompetitionSponsorGroupsFn({
        data: {competitionId: 'comp-1'},
      })

      expect(result.groups).toHaveLength(2)
      expect(result.groups[0].name).toBe('Gold')
      expect(result.groups[1].name).toBe('Silver')
    })
  ...
  describe('getUserSponsorsFn', () => {
    it('returns ordered sponsors for user', async () => {
      ...
      const result = await getUserSponsorsFn({data: {userId: 'user-1'}})

      expect(result.sponsors).toHaveLength(2)
      expect(result.sponsors[0].name).toBe('Athlete Sponsor 1')
      expect(result.sponsors[1].name).toBe('Athlete Sponsor 2')
    })
  ...
  describe('createSponsorGroupFn', () => {
    ...
    it('creates sponsor group with explicit order', async () => {
      ...
      expect(result.group.displayOrder).toBe(2)
    })
  ...
  describe('updateSponsorGroupFn', () => {
    it('updates sponsor group name', async () => {
      ...
      // update().set().where() is awaited, consuming a where mock
      whereMock.mockResolvedValueOnce([])
      // Select-back after update
    ...
    it('updates display order', async () => {
      ...
      expect(result.group?.displayOrder).toBe(5)
    })
  ...
  describe('reorderSponsorGroupsFn', () => {
    it('updates display order for multiple groups', async () => {
      ...
      const result = await reorderSponsorGroupsFn({
        data: {
          competitionId: 'comp-1',
  ...
  describe('createSponsorFn', () => {
    ...
    it('creates sponsor with group assignment', async () => {
      ...
      expect(result.sponsor.groupId).toBe('group-1')
    })
    ...
    it('appends to end of existing sponsors', async () => {
      ...
      expect(result.sponsor.displayOrder).toBe(2)
    })
  })

  describe('updateSponsorFn', () => {
    it('updates sponsor name', async () => {
      ...
      whereMock.mockResolvedValueOnce([existing])   // get existing sponsor
      ...
      expect(result.sponsor?.name).toBe('New Name')
    })
    ...
    it('updates sponsor group assignment', async () => {
      ...
      expect(result.sponsor?.groupId).toBe('new-group')
    })
    ...
    it('removes sponsor from group', async () => {
      ...
      expect(result.sponsor?.groupId).toBeNull()
    })
    ...
    it('updates logo URL', async () => {
      ...
      whereMock.mockResolvedValueOnce([existing])
      // No competition lookup for user sponsors
      // update().set().where() is awaited, consuming a where mock
      whereMock.mockResolvedValueOnce([])
      // select-back after update
      whereMock.mockResolvedValueOnce([updated])
      ...
      expect(result.sponsor?.logoUrl).toBe('https://example.com/logo.png')
    })
  ...
  describe('reorderSponsorsFn', () => {
    it('reorders sponsors within competition', async () => {
      ...
      const result = await reorderSponsorsFn({
        data: {
          competitionId: 'comp-1',
          sponsorOrders: [
  ...
  describe('assignWorkoutSponsorFn', () => {
    it('assigns sponsor to track workout', async () => {
      ...
      const result = await assignWorkoutSponsorFn({
        data: {
          trackWorkoutId: 'tw-1',
          competitionId: 'comp-1',
          sponsorId: 'sponsor-1',
  ...
  describe('getWorkoutSponsorFn', () => {
    it('returns sponsor for track workout', async () => {
      ...
      const result = await getWorkoutSponsorFn({
    ...
    it('returns null when track workout not found', async () => {
      ...
      const result = await getWorkoutSponsorFn({
        data: {trackWorkoutId: 'nonexistent'},
      })
