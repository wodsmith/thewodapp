...
it("adds private completion and instruction sections only on explicit save", async () => {
 for (const kind of ["check", "note"] as const) {
  ...
  const { unmount } = render(<AthletePersonalSession surface="session" team={team} trackId="track" date={session.trainingDate} sourceResults={[]} onSaved={vi.fn()} />)
  fireEvent.click(await screen.findByRole("button", { name: "Customize session" }))
  fireEvent.click(screen.getByRole("button", { name: "Add instructions or completion" }))
  expect(savePersonalTrainingSessionFn).not.toHaveBeenCalled()
