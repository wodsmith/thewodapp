...
2. Copy your **Test mode** publishable and secret keys.
...
3. Copy your **Test mode client ID**
...
VITE_STRIPE_PUBLISHABLE_KEY=replace_me_stripe_publishable_key

# Stripe Connect OAuth (for organizer payouts)
STRIPE_CLIENT_ID=replace_me_stripe_connect_client_id
...
```
> Ready! Your webhook signing secret is shown here (^C to quit)
```

**Copy the webhook signing secret value** and set it as `STRIPE_WEBHOOK_SECRET` in your `.dev.vars`:
