 ...
 * invite emails (to not-yet-registered invitees), plus Crew assignment
 * confirmations/reminders. The shared queue binding `BROADCAST_EMAIL_QUEUE`
 * carries each shape; the consumer dispatches on the `kind` discriminator.
 *
...
async function handleCrewAssignmentEmailMessage(params: {
  message: MessageBatch["messages"][number]
  body: CrewAssignmentEmailQueueMessage
  ...
  if (!resendApiKey) {
    logInfo({
      message: "[CrewEmailQueue] No RESEND_API_KEY — logging (dev mode)",
      attributes: {
  ...
  logInfo({
    message: "[CrewEmailQueue] Processing assignment email",
    attributes: {
      confirmationId: body.confirmationId,
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        tags: [{ name: "type", value: body.kind }],
      }),
    })
    ...
    if (!response.ok) {
      ...
      logError({
        message: "[CrewEmailQueue] Resend API error",
        attributes: {
          confirmationId: body.confirmationId,
    ...
    logError({
      message: "[CrewEmailQueue] Email send failed",
      error: err,
      attributes: {
        confirmationId: body.confirmationId,
        sendSucceeded,
...
async function markCrewAssignmentEmailDelivered(params: {
  db: ReturnType<typeof getDb>
  body: CrewAssignmentEmailQueueMessage
}) {
  const queuedAt = new Date(params.body.queuedAtIso)
  const deliveredAt = Number.isNaN(queuedAt.getTime()) ? new Date() : queuedAt

  if (params.body.kind === "crew-assignment-confirmation") {
    await params.db
      ...
      .where(
        eq(crewAssignmentConfirmationsTable.id, params.body.confirmationId),
      )
    return
  }
  ...
  await params.db
    .update(crewAssignmentConfirmationsTable)
    .set({
      lastReminderAt: deliveredAt,
      reminderCount: params.body.reminderCount,
      updatedAt: deliveredAt,
    })
    .where(eq(crewAssignmentConfirmationsTable.id, params.body.confirmationId))
}
  ...
  if (!resendApiKey) {
    ...
    if (recipientIds.length > 0) {
      await db
        ...
        .where(inArray(competitionBroadcastRecipientsTable.id, recipientIds))
    }
    message.ack()
    return
  }
    ...
    try {
      ...
      const alreadySent = new Set(
        existingRecipients
          .filter(
            (r) =>
              r.emailDeliveryStatus === BROADCAST_EMAIL_DELIVERY_STATUS.SENT,
          )
          .map((r) => r.id),
      )
      ...
      for (const recipient of pendingRecipients) {
        try {
          ...
          if (!response.ok) {
            const errorBody = await response.text().catch(() => "unknown")
            logError({
              message: "[BroadcastQueue] Resend API error",
              attributes: {
      ...
      const sentIds = results.filter((r) => r.success).map((r) => r.recipientId)
      ...
      if (sentIds.length > 0) {
        await db
          ...
          .where(inArray(competitionBroadcastRecipientsTable.id, sentIds))
      }
      ...
      if (failedIds.length > 0) {
        await db
          ...
          .where(inArray(competitionBroadcastRecipientsTable.id, failedIds))
      }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      await db
        .update(competitionInvitesTable)
        .set({
          emailDeliveryStatus: COMPETITION_INVITE_EMAIL_DELIVERY_STATUS.FAILED,
