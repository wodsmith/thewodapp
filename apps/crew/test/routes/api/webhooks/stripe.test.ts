  ...
  describe('charge.refunded', () => {
    ...
    it('records REFUND_COMPLETED for each succeeded refund on the charge', async () => {
      const charge = {
        ...
        refunds: {
          data: [
            {id: 'refund_test_1', amount: 7500, status: 'succeeded', reason: null},
          ],
        },
      }
    ...
    it('skips refunds whose status is not "succeeded"', async () => {
      const charge = {
        ...
        refunds: {
          data: [
            {id: 'refund_pending', amount: 5000, status: 'pending', reason: null},
            {id: 'refund_failed', amount: 5000, status: 'failed', reason: null},
          ],
        },
      }
    ...
    it('is idempotent: skips refunds already recorded as REFUND_COMPLETED', async () => {
      const charge = {
        ...
        refunds: {
          data: [
            {id: 'refund_dupe', amount: 7500, status: 'succeeded', reason: null},
          ],
        },
      }
      const event = buildStripeEvent('charge.refunded', charge)
    ...
    it('returns 200 and does nothing when payment_intent cannot be resolved', async () => {
      const charge = {
        ...
        refunds: {
          data: [
            {id: 'refund_orphan', amount: 1000, status: 'succeeded', reason: null},
          ],
        },
      }
      const event = buildStripeEvent('charge.refunded', charge)
