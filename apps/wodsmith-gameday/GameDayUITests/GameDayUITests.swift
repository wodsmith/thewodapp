...
final class GameDayUITests: XCTestCase {
    ...
    func testAthleteCanOpenPersonalScheduleAndCompetitionPages() {
        ...
        XCTAssertTrue(app.buttons["Heats, My heats"].waitForExistence(timeout: 5))
        app.buttons["Heats, My heats"].tap()
        app.buttons["All heats"].tap()
        XCTAssertTrue(app.buttons["Heats, All heats"].waitForExistence(timeout: 5))
