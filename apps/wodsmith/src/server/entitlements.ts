...
import { getDd } from "../utils/get-db"
import { ZSAError } from "@repo/zsa/errors"
...
export async function requireLimitExcludingPersonalTeams(
	...
	// Get user's plan limit from their first team's plan (or use free plan)
	// NOTE: This assumes all teams owned by the user share the same plan
	// In the future, this could be user-level instead of team-level
	const userTeam = await db.query.teamTable.findFirst({
		where: eq(teamTable.personalTeamOwnerId, userId),
	})

	if (!userTeam) {
		// If no team found, use free plan limits
		const maxTeams = PLANS.FREE.entitlements.limits[limitKey]
	...
	const userPlan = await getTeamPlan(userTeam.id)
	const maxTeams = userPlan.entitlements.limits[limitKey]
