...
export async function checkCanInviteMember(
	...
	if (maxMembers === undefined || maxMembers === null) {
		throw new Error(
			`MAX_MEMBERS_PER_TEAM limit not found in database for team ${teamId}. Run seed script: pnpm db:seed`,
		)
	}
...
export async function checkCanCreateProgrammingTrack(
	...
	if (maxTracks === undefined || maxTracks === null) {
		throw new Error(
			`MAX_PROGRAMMING_TRACKS limit not found in database for team ${teamId}. Run seed script: pnpm db:seed`,
		)
	}
...
export async function checkCanUseAI(
	...
	if (maxMessages === undefined || maxMessages === null) {
		throw new Error(
			`AI_MESSAGES_PER_MONTH limit not found in database for team ${teamId}. Run seed script: pnpm db:seed`,
		)
	}
