...
import { handleOAuthCallback, parseOAuthState } from "@/server/stripe-connect"
...
export async function GET(request: NextRequest) {
	...
	if (error) {
		...
		try {
			...
			return createRedirect("/compete/organizer?error=oauth_failed", appUrl)
		}
	}
	...
	try {
		...
		return createRedirect("/compete/organizer?error=invalid_state", appUrl)
	}

	// Validate CSRF token from cookie
	const cookieStore = await cookies()
	const csrfCookie = cookieStore.get(STRIPE_OAUTH_STATE_COOKIE_NAME)?.value
	...
	cookieStore.delete(STRIPE_OAUTH_STATE_COOKIE_NAME)
	...
	if (!session) {
		...
		// Redirect to login with return URL to the payouts page
 ...
 *
 * Using Response with status 307 and Location header instead of NextResponse.redirect()
