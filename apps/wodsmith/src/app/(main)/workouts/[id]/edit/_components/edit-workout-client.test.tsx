...
vi.mock("@repo/zsa-react", () => ({
	useServerAction: () => ({
		execute: vi
			.fn()
			.mockResolvedValue([{ success: true, data: { levels: [] } }]),
		isPending: false,
		isSuccess: false,
		isError: false,
		data: undefined,
...
vi.mock("@/components/movements-list", () => ({
	MovementsList: vi.fn(() => (
		<div data-testid="movements-list">Movements List</div>
	)),
}))

// Mock WorkoutScalingDescriptionsEditor component
vi.mock("@/components/scaling/workout-scaling-descriptions-editor", () => ({
	WorkoutScalingDescriptionsEditor: vi.fn(() => (
		<div data-testid="scaling-editor">Scaling Editor</div>
	)),
}))
