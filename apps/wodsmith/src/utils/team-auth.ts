 ...
 * @throws ZSAError if not authenticated or lacks both permission and admin role
 */
export const requireTeamPermissionOrAdmin = cache(
	async (teamId: string, permission: string) => {
		const session = await requireVerifiedEmail()
		...
		// Import dynamically to check admin status
 ...
 */
export const canHostCompetitions = cache(
 ...
 */
export const isCompetitionEventTeam = cache(
	async (teamId: string): Promise<boolean> => {
 ...
 */
export const getParentOrganizationId = cache(
	async (competitionTeamId: string): Promise<string | null> => {
		const { getDb } = await import("@/db")
		const { teamTable } = await import("@/db/schema")
		const { eq } = await import("drizzle-orm")
