...
test.describe("Competition Registration with Waivers", () => {
	...
	const competitionSlug = `e2e-test-comp-${timestamp}`
	const competitionId = `e2e_comp_${timestamp}`
	const divisionId = `e2e_div_${timestamp}`
	const waiverId = `e2e_waiver_${timestamp}`
	...
	test("should complete registration flow with waiver signing", async ({
		...
		if (await continueButton.isVisible({ timeout: 2000 })) {
			...
			await expect(
				successText.or(paymentForm).first(),
