...
CREATE TABLE `__new_workouts` (
	...
	FOREIGN KEY (`source_track_id`) REFERENCES `programming_track`(`id`) ON UPDATE no action ON DELETE no action,
	FOREIGN KEY (`source_workout_id`) REFERENCES `workouts`(`id`) ON UPDATE no action ON DELETE no action
);
