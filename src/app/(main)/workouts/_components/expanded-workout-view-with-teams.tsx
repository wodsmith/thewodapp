...
interface ExpandedWorkoutViewProps {
	workouts: ScheduledWorkoutWithTeam[]
...
export function ExpandedWorkoutView({
				...
				<CardContent>
					<div className="text-center py-8">
						...
						<p className="text-muted-foreground">
							You have no workouts scheduled for{" "}
							{isToday ? "today" : "this day"} across any of your teams.
						</p>
					</div>
				</CardContent>
			</Card>
	...
	// Group workouts by team
	const workoutsByTeam = workouts.reduce(
		(acc, workout) => {
			if (!acc[workout.teamId]) {
				acc[workout.teamId] = {
					teamName: workout.teamName,
					teamSlug: workout.teamSlug,
					workouts: [],
				}
			}
			acc[workout.teamId].workouts.push(workout)
			return acc
		},
		{} as Record<
			string,
	...
	return (
		<Card>
			...
			<CardContent className="space-y-6">
				{Object.entries(workoutsByTeam).map(([teamId, teamData], teamIndex) => (
					<div key={teamId}>
						{teamIndex > 0 && <Separator className="my-6" />}

						<div className="space-y-4">
							{teamData.workouts.map((workout, index) => (
								<div key={workout.id}>
