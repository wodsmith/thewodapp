...
const TimeSlotManager = ({ timeSlots, setTimeSlots, locations, classTypes }: TimeSlotManagerProps) => {
  const [newTimeSlot, setNewTimeSlot] = useState({ location: "", classType: "", time: "" });
  ...
  const removeTimeSlot = (location: string, classType: string, time: string) => {
  ...
  return (
    <Card className="bg-white/60 backdrop-blur-sm border-white/20 mb-6">
      <CardHeader>
        ...
        <CardDescription>Configure custom time slots for each location and class type</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-lg">
          <div>
            ...
            <Select value={newTimeSlot.location} onValueChange={(value) => setNewTimeSlot({...newTimeSlot, location: value})}>
              <SelectTrigger>
          ...
          <div>
            ...
            <Select value={newTimeSlot.classType} onValueChange={(value) => setNewTimeSlot({...newTimeSlot, classType: value})}>
              <SelectTrigger>
                ...
                <div className="flex flex-wrap gap-2">
                  {slots.map((time) => (
                    <Badge key={time} variant="secondary" className="flex items-center space-x-1">
                      <span>{time}</span>
                      <button onClick={() => removeTimeSlot(location, classType, time)}>
