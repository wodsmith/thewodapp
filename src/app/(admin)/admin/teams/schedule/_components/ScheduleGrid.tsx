...
const ScheduleGrid = ({ schedule, setSchedule, timeSlots, locations, currentWeek }: ScheduleGridProps) => {
  const [selectedSlot, setSelectedSlot] = useState<{ key: string; slot: ScheduleSlot } | null>(null);
  ...
  const renderScheduleSlot = (day: string, time: string, location: string) => {
    ...
    if (!slot) {
      return <div className="h-16 border border-slate-200 rounded bg-white/50"></div>;
    }
      ...
      >
        <div className="flex items-center justify-between h-full">
          <div className="flex-1 min-w-0">
            <div className="font-medium text-slate-800 truncate">{slot.class}</div>
            {slot.coach ? (
    ...
    <>
      <Card className="bg-white/60 backdrop-blur-sm border-white/20">
        <CardHeader>
          ...
          <CardDescription>Current schedule for {currentWeek} - Click on any slot to assign or change coaches</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              return (
                <div key={location} className="mb-8">
                  ...
                  <div className="grid grid-cols-8 gap-2 min-w-[800px]">
                    {/* Header row */}
                    <div className="font-medium text-slate-600 text-sm p-2">Time</div>
                    {days.map((day) => (
                      <div key={day} className="font-medium text-slate-600 text-sm p-2 text-center">
                        {day}
                      </div>
                    ))}
