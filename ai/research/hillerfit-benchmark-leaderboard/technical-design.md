...
- The first seeded battery uses `videoPolicy: "never"` and `isOpenJoin: false` as seed defaults; the later M3 write path is responsible for published/visible, profile, waiver, idempotency, and rate-limit guards before any battery enables open-join submissions.
