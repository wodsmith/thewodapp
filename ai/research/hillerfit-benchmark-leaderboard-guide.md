...
interface CompetitionTypeDef {
  ...
  selectableOnCreate: boolean   // benchmark is further gated by team entitlement
}
...
export const COMPETITION_TYPE_REGISTRY: Record<string, CompetitionTypeDef> = {
  ...
  "benchmark": { id: "benchmark", label: "Benchmark", leaderboardVariant: "online", selectableOnCreate: true,
...
| type **picker / filter / create-update** | the generic picker reads `selectableOnCreate`, then hides benchmark unless the selected team has the creation entitlement | `organizer-competition-form`, `*-edit-form`, `routes/index`, `compete/index` | ~10 |
...
- **Training-guide-derived benchmark** ships as a seeded battery (mirror `demo-competition-fns.ts`): 58 designed trackWorkouts tagged with `benchmarkTestId` + `benchmarkCategory`, one "Open" division, the battery + tests + thresholds rows. The seed extracts its test and threshold data from the owner-provided `HillerFit_Training_Guide.pdf` artifact. Each test carries the correct `scheme` + `scoreType` (the latter encodes direction — §6.1). It does **not** create any HillerFit-branded page, route, navigation item, marketing surface, logo, call to action, or theme.
